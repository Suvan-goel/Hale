...
PEARL UNIFIED MOVEMENT CHECK-UP — STAGE H1
...
Its purpose is to prove that Pearl’s polished current `CheckUpScreen` visual experience can run the existing V2 live measurement protocol without changing the public/default Check-Up route yet.
...
```bash
rm -rf /tmp/pearl-unified-h1-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-unified-h1-baseline-export
rc=$?
rm -rf /tmp/pearl-unified-h1-baseline-export
exit $rc
```
...
```bash
rm -rf /tmp/pearl-unified-h1-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-unified-h1-export
rc=$?
rm -rf /tmp/pearl-unified-h1-export
exit $rc
```
