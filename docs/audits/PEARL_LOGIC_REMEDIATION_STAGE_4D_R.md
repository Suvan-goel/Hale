# Pearl Logic Remediation Stage 4D-R
...
Primary Stage 4D-R files changed or added: `src/training/safetyCueDefinitions.ts`, `src/training/safetyCues.ts`, `src/training/sessionPlayer.ts`, `src/screens/TrainingSessionScreen.tsx`, `src/screens/SessionPreviewScreen.tsx`, `src/pearlFlow/sessionPlanning.ts`, `src/pearlFlow/types.ts`, `src/pearlFlow/exploreViewModel.ts`, `src/audio/cues.ts`, `src/audio/voicePlayer.ts`, `scripts/generate-audio.ts`, tests, and `docs/decisions.md`.
