You are implementing the final software-hardening stage of Hale’s unified Movement Check-Up migration:
...
```bash
rm -rf /tmp/hale-unified-h5d-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h5d-baseline-export
rc=$?
rm -rf /tmp/hale-unified-h5d-baseline-export
exit $rc
```
...
env <safe flags> npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h5d-safe-beta-export
rc=$?
rm -rf /tmp/hale-unified-h5d-safe-beta-export
exit $rc
```
