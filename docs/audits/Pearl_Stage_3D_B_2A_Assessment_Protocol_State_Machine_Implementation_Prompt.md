You are implementing Stage 3D-B.2A of Pearl’s production-readiness work:
...
- balance raw seconds plus Pearl task band and source benchmark detail later;
...
```bash
rm -rf /tmp/pearl-stage3db2a-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-stage3db2a-export
rc=$?
rm -rf /tmp/pearl-stage3db2a-export
exit $rc
```
