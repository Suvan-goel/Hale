# Hale Unified Movement Check-Up - Stage H5D Release-Candidate Hardening
...
Required H5C report exists at `docs/audits/HALE_UNIFIED_MOVEMENT_CHECKUP_STAGE_H5C_PUBLIC_V1_ROUTE_RETIREMENT.md`.
...
```bash
rm -rf /tmp/hale-unified-h5d-safe-beta-export
env EXPO_NO_DOTENV=1 \
  EXPO_PUBLIC_ENABLE_LEGACY_V1_CHECKUP_ROLLBACK=0 \
  EXPO_PUBLIC_ENABLE_MOVEMENT_PROFILE_V2_INTERNAL=0 \
  EXPO_PUBLIC_ENABLE_MOVEMENT_PROFILE_V2_DIAGNOSTICS=0 \
  EXPO_PUBLIC_ENABLE_POSE_LATENCY_DIAGNOSTICS=0 \
  EXPO_PUBLIC_ALLOW_DIAGNOSTICS_IN_RELEASE=0 \
  npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h5d-safe-beta-export
rc=$?
rm -rf /tmp/hale-unified-h5d-safe-beta-export
exit $rc
```
...
```text
docs/qa/HALE_CLOSED_BETA_PHYSICAL_DEVICE_QA_PROTOCOL.md
```
...
```bash
rm -rf /tmp/hale-unified-h5d-normal-export
npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h5d-normal-export
rc=$?
rm -rf /tmp/hale-unified-h5d-normal-export
exit $rc
```
