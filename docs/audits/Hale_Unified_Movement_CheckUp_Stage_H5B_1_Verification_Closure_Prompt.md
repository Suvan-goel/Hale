You are carrying out the evidence-closure stage for Hale’s Balanced micro-check implementation:

HALE UNIFIED MOVEMENT CHECK-UP — STAGE H5B.1
...
```bash
rm -rf /tmp/hale-unified-h5b1-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h5b1-baseline-export
rc=$?
rm -rf /tmp/hale-unified-h5b1-baseline-export
exit $rc
```
...
src/haleFlow/__tests__/microCheckLifecycleH5B1.integration.test.ts
