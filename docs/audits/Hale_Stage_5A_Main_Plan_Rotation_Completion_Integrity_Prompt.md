You are implementing Stage 5A of Hale’s production-readiness work:
...
When `planTodayHaleSession` or equivalent receives an explicit planning date:
...
Do not leave independent ad hoc definitions of “main plan” in App, HaleFlow, adherence, and training.
...
- `planTodayHaleSession({ today })` passes the date to adaptation.
...
- “Hale saved the attempt, but it did not count as a planned session.”
