You are carrying out Stage 3D-B.1 of Pearl’s production-readiness work:
...
1. Pearl should move away from exact “movement age” as the primary controlled-beta result.

2. Pearl should preserve the motivating comparison experience through:
...
- a modern age- and sex-specific 30-second sit-to-stand reference source may support percentile or centile comparison across Pearl’s target ages;
- a 45-second one-leg stance reference protocol may support age-group comparison if Pearl’s protocol is aligned;
...
A modern age- and sex-specific 30-second sit-to-stand normative/centile source covering adults through Pearl’s 45–65 target range, including the Warden et al. source identified in the follow-up analysis.
...
Do not force a source to fit Pearl’s existing table.
...
Temporary read-only probes and source files may exist only under `/tmp/pearl-stage3db1/` and must be removed.
...
Must be labelled as Pearl’s product interpretation, not a population norm.
...
- Can Pearl compute exact percentiles locally from published parameters?
- Can Pearl lawfully reproduce the required parameters?
...
```text
Based on a similar published test setup. Pearl’s camera estimate is not a medical assessment.
```
...
Determine whether Pearl currently stores:
