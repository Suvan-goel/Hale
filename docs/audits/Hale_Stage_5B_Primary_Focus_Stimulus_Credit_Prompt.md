You are implementing Stage 5B of Hale’s production-readiness work:
...
Stage 5B must combine these two contracts so that Hale only advances a main-plan session when the user completed meaningful work for the block’s actual focus.
...
- “Hale couldn’t include a primary [focus] exercise with today’s setup.”
...
- “Hale recorded your supporting work, but this session did not include completed [focus] work, so your plan has not moved forward.”
...
- Accept the actual `HaleSessionPlan`/generated metadata and active block focus.
