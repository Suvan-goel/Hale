You are implementing the third hands-free remediation stage for Pearl:

PEARL HANDS-FREE FLOW — STAGE HF3
...
After the user starts a training session and places the phone, Pearl should not require screen interaction to progress through ordinary exercise setup and set start when camera-readiness can safely detect that the user is ready.
...
```bash
rm -rf /tmp/pearl-hf3-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-hf3-baseline-export
rc=$?
rm -rf /tmp/pearl-hf3-baseline-export
exit $rc
```
...
```bash
rm -rf /tmp/pearl-hf3-hands-free-training-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-hf3-hands-free-training-export
rc=$?
rm -rf /tmp/pearl-hf3-hands-free-training-export
exit $rc
```
