You are implementing the next lifecycle stage of Pearl’s approved unified Movement Check-Up architecture:
...
```text
This result used a different standing leg, so Pearl is showing it separately rather than comparing it directly.
```
...
```bash
rm -rf /tmp/pearl-unified-h4-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-unified-h4-baseline-export
rc=$?
rm -rf /tmp/pearl-unified-h4-baseline-export
exit $rc
```
...
```text
Your next 4-week plan is ready
Pearl prepared it from your latest Movement Profile.
```
...
```text
A different standing leg was used this time, so Pearl is showing the current balance result separately.
```

```text
A different shoulder was tested this time, so Pearl is showing the current reach result separately.
```
...
```bash
rm -rf /tmp/pearl-unified-h4-export
