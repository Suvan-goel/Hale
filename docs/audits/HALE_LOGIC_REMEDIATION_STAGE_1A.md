# Hale Logic Remediation Stage 1A
...
- Tests under `src/adherence`, `src/haleFlow`, and `src/onboarding`.
...
- `npm test -- --runInBand src/haleFlow/__tests__/assessmentEligibility.test.ts src/haleFlow/__tests__/assessmentResultState.test.ts src/adherence/__tests__/blockServiceEligibility.test.ts src/onboarding/__tests__/onboarding.test.ts src/haleFlow/__tests__/appLifecycle.test.ts`
  - Exit 0
...
- `npx --no-install expo config --json`
  - Exit 0.
  - Parsed config: name `Hale`, slug `hale`, scheme `hale`, 6 plugins, iOS `com.suvangoel.hale`, Android `com.suvangoel.hale`.
- `git diff --check`
  - Exit 0.
...
`assets/images/hale-home-hero-premium.png` appeared as an unrelated untracked file during the task and was not touched by this remediation.
