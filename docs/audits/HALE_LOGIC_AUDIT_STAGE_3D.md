# Hale Logic Audit Stage 3D: Norm Provenance, Claims, Ties, and Meaningful Change

Date: 2026-06-20
Mode: read-only audit and product-decision packet
Output: `docs/audits/HALE_LOGIC_AUDIT_STAGE_3D.md`
...
$ git diff --stat
 ...
 src/haleFlow/__tests__/appLifecycle.test.ts        |  83 ++--
 .../__tests__/assessmentEligibility.test.ts        | 138 +++++--
 src/haleFlow/__tests__/haleFlow.test.ts            |  32 +-
 src/haleFlow/__tests__/progressViewModel.test.ts   | 309 ++++++++++++++-
 src/haleFlow/__tests__/sessionPlanning.test.ts     |  14 +-
 src/haleFlow/appLifecycle.ts                       |  24 +-
 src/haleFlow/assessmentEligibility.ts              | 183 +++++++--
 src/haleFlow/assessmentResultState.ts              |  48 ++-
 src/haleFlow/assessments.ts                        |  94 +++--
 src/haleFlow/index.ts                              |   2 +
 src/haleFlow/progressViewModel.ts                  | 124 ++++--
 src/haleFlow/reports.ts                            |  67 +++-
...
1. `createMovementAssessment` computes headline evidence completeness and only publishes an authoritative focus when all three headline domains are complete (`src/haleFlow/assessments.ts:16-44`).
2. `getBlockCreationEligibility` requires a completed official assessment, complete headline evidence, a focus domain, finite focus measurement, a current compatible snapshot, and score/assessment source identity (`src/haleFlow/assessmentEligibility.ts:42-118`).
...
| Domain band thresholds | midpoint age -> strong/building/starting point | `src/haleFlow/progressViewModel.ts:420-424`; similar lifecycle/onboarding logic | midpoint age | band | No external source | Product heuristic | N/A | N/A | N/A | N/A | No | Yes/yes | Safe only as product bands with caveat, not normative truth |
| Meaningful-change thresholds | raw/age deltas -> improved/held/lower | `src/haleFlow/reports.ts:140-145`; `src/haleFlow/progressViewModel.ts:272-346` | years, reps, seconds, degrees, body units | trend labels | No external source | Product heuristic | N/A | N/A | N/A | N/A | No | Yes/yes | Beta-blocked for improvement/decline claims until repeatability policy |
...
- Obtain the exact Bohannon single-limb stance table, truncation rules, eyes-open protocol, and whether max trial duration matches Hale's 12-second stage.
- Resolve the mismatch between Hale's 12-second single-leg stage and norm anchors of 27-35 seconds.
...
| 7 | "Typical ages" / "Estimated typical ages" | `src/haleFlow/progressViewModel.ts:402-407` | Latest evidence | Snapshot age range | Varies | Beta label and provenance | Norm overclaim | Beta-label | Use "Beta estimated range" |
| 8 | "Main opportunity: Domain" | `src/haleFlow/progressViewModel.ts:464-468` | Latest score | `weakestDomain` | Medium | tie/near-tie | Overconfident | Soften | "Suggested focus" or "closely matched" |
...
| 10 | "That's N more strong rises" | `src/haleFlow/progressViewModel.ts:272-283` | Chair reps delta >0 | Raw reps | Medium | smallest detectable change | Improvement overclaim if 1 rep not reliable | Soften | Use threshold or "more reps recorded" |
| 11 | "That's N more steady seconds" | `src/haleFlow/progressViewModel.ts:302-313` | Balance delta >0 | Raw hold seconds | Low-medium | hold repeatability | "steady" overclaim | Soften | Use threshold and raw copy |
| 12 | "Your mobility check is improving" | `src/haleFlow/progressViewModel.ts:332-346` | Shoulder >0 deg or reach >0.01 bu | Raw shoulder/hinge | Low | repeatability | Tiny metric overclaim | Soften | Neutral until threshold |
...
| 16 | "protects progress" / "stay capable" | `src/adherence/screens/BlockReportScreen.tsx:83-91`; `src/haleFlow/copy.ts:102-106` | Goal/report/adherence | Training state | Low-med | no guarantee | Capability promise | Soften | "supports the progress you are working on" |
...
| 22 | "area Hale measured first" | `src/screens/PlanScreen.tsx:254-260` | Active block | active focus | Incorrect | should be weakest/focus area | Misleading | Remove | Replace in future remediation |
...
| 24 | "measure strength, balance, and mobility and build your first plan" | `src/haleFlow/copy.ts:47-53` | First baseline CTA | planned scoring | Medium | beta estimate/camera limitation | Broad measurement claim | Soften | "estimate" or "check" |
...
- `createMovementBlockReport` only compares when snapshots are compatible and source endpoints match (`src/haleFlow/reports.ts:44-68`).
- For compatible pairs, it computes midpoint age per domain (`src/haleFlow/reports.ts:118-137`).
- `current < previous` means improved, exact equality means held steady, and any greater value means declined (`src/haleFlow/reports.ts:140-145`).
...
- Strength: any positive rep delta is improved, exactly zero held steady, any negative lower (`src/haleFlow/progressViewModel.ts:258-284`).
- Balance: any positive seconds delta is improved, absolute delta under 0.5 is held steady only when non-positive, otherwise lower (`src/haleFlow/progressViewModel.ts:287-314`).
- Mobility: shoulder delta >0 or hinge reach improvement >0.01 bu is improved; any shoulder decrease or reach decrease below -0.01 is lower (`src/haleFlow/progressViewModel.ts:317-347`).
- Balance progress cards may prefer tandem hold over single-leg hold (`src/haleFlow/progressViewModel.ts:373-380`), while balance age uses single-leg only. Hinge can drive mobility progress while mobility age ignores hinge.
...
No current audited copy uses "fall risk", "frailty diagnosis", "medical-grade", "treatment", or disease-prevention claims. Keep the safety copy "Hale is not medical care" as-is.
...
- `src/services/backend/__tests__/checkupSyncService.test.ts`
- `src/services/backend/__tests__/restoreService.test.ts`
- copy guardrail tests including Hale flow/adherence/preflight/exercise copy tests.
...
Recommendation: show "home movement estimate", "beta estimate", "not a medical assessment", "not a diagnosis", and "small changes may reflect setup/noise; Hale looks for repeatable change over time." Avoid "medical-grade", "clinical result", "fall risk", "guaranteed independence", and authoritative age labels.
...
$ git diff --stat
 ...
 src/haleFlow/__tests__/appLifecycle.test.ts        |  83 ++-
 .../__tests__/assessmentEligibility.test.ts        | 138 ++++-
 src/haleFlow/__tests__/copyGuardrails.test.ts      |   2 +
 src/haleFlow/__tests__/exploreViewModel.test.ts    |  21 +-
 src/haleFlow/__tests__/haleFlow.test.ts            |  32 +-
 src/haleFlow/__tests__/progressViewModel.test.ts   | 309 ++++++++++-
 src/haleFlow/__tests__/sessionPlanning.test.ts     |  14 +-
 src/haleFlow/appLifecycle.ts                       |  24 +-
 ...
 src/haleFlow/assessmentResultState.ts              |  48 +-
 src/haleFlow/assessments.ts                        |  94 +++-
 src/haleFlow/exploreViewModel.ts                   |  98 ++++
 src/haleFlow/index.ts                              |   2 +
 src/haleFlow/progressViewModel.ts                  | 124 +++--
 src/haleFlow/reports.ts                            |  67 ++-
 src/haleFlow/types.ts                              |   3 +-
