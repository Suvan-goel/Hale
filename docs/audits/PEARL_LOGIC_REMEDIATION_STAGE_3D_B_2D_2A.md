# Pearl Logic Remediation Stage 3D-B.2D.2A
...
```bash
rm -rf /tmp/pearl-stage3db2d2a-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-stage3db2d2a-export
rc=$?
rm -rf /tmp/pearl-stage3db2d2a-export
exit $rc
```
