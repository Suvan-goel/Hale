# Pearl Logic Audit - Stage 4 Exercise Catalogue
...
### git status --short --untracked-files=all
 ...
 M CLAUDE.md
 M app.json
 M assets/images/pearl-home-hero-premium.png
 M docs/decisions.md
...
?? docs/audits/Pearl_Stage_3D_C_Exact_Tie_Policy_Prompt.md
?? docs/audits/Pearl_Stage_3D_D_Near_Tie_Meaningful_Change_Prompt.md
?? docs/audits/Pearl_Stage_3D_Norm_Claims_Audit_Prompt.md
?? docs/audits/Pearl_Stage_4_Exercise_Catalogue_Audit_Prompt.md
...
### git diff --stat
 ...
 CLAUDE.md                                          |    2 +-
 app.json                                           |    6 +-
 assets/images/pearl-home-hero-premium.png           |  Bin 1753564 -> 1880702 bytes
 ...
 src/pearlFlow/__tests__/appLifecycle.test.ts        |   84 +-
 .../__tests__/assessmentEligibility.test.ts        |  419 +++++++-
 src/pearlFlow/__tests__/copyGuardrails.test.ts      |   56 +-
 src/pearlFlow/__tests__/exploreViewModel.test.ts    |   35 +-
 src/pearlFlow/__tests__/pearlFlow.test.ts            |   32 +-
 src/pearlFlow/__tests__/progressViewModel.test.ts   |  327 ++++++-
 src/pearlFlow/__tests__/sessionPlanning.test.ts     |   14 +-
 src/pearlFlow/appLifecycle.ts                       |   41 +-
 src/pearlFlow/assessmentEligibility.ts              |  267 +++++-
 src/pearlFlow/assessmentResultState.ts              |   48 +-
 src/pearlFlow/assessments.ts                        |  119 ++-
 src/pearlFlow/copy.ts                               |   18 +-
 src/pearlFlow/exploreViewModel.ts                   |  233 ++++-
 src/pearlFlow/index.ts                              |    2 +
 src/pearlFlow/planViewModel.ts                      |    6 +-
 src/pearlFlow/progressViewModel.ts                  |  202 ++--
 src/pearlFlow/reports.ts                            |   75 +-
...
Third, session assembly lives mostly in `src/training/workoutGeneration.ts`. It defines domain templates, extra-session presets, slot fallback ladders, equipment filtering, pain filtering, readiness adjustments, generated exercise payloads, and post-session ladder progression. `src/pearlFlow/sessionPlanning.ts` adapts generated sessions into `PearlSessionPlan`, supports manual ladder practice, and still has a legacy fallback path. `src/pearlFlow/exploreViewModel.ts` exposes extra-session cards and ladder cards/details.
...
- `floor` is treated as always supported by equipment filtering (`src/training/workoutGeneration.ts:964-981`) and is filtered out of session equipment labels (`src/pearlFlow/sessionPlanning.ts:843-848`). That hides an important access and safety requirement for bridge and floor push-up work.
...
1. Floor-based work is allowed without floor-transfer screening. `glute-bridge-hold`, `glute-bridge-reps`, and `push-up-standard` require floor access, but `floor` always passes equipment support and is omitted from equipment labels (`src/training/workoutGeneration.ts:964-981`, `src/pearlFlow/sessionPlanning.ts:843-848`).
2. Step-up is a core strength path when stairs are available, but the catalogue lacks explicit step-height, rail/counter support, stair surface, or space requirements (`src/exercises/ladders.ts:240-263`). The measurement note says Pearl does not score foot placement or stair height.
...
Explore cards and ladder detail reuse the live catalogue. Extra-session cards are generated from presets and disabled when a band or stair-specific preset lacks the required equipment (`src/pearlFlow/exploreViewModel.ts:156-191`). Ladder cards list visible `v1_core` ladders and detail pages filter optional levels out (`src/pearlFlow/exploreViewModel.ts:194-245`).

Manual ladder practice is wired through `planLadderPracticeSession`, selects the current/default/core equipment-compatible level, creates a one-exercise plan, and adds generic guidance (`src/pearlFlow/sessionPlanning.ts:193-243`).
...
- Session planning and Explore view-model behavior in `src/pearlFlow/__tests__`.
