# Codex task: Build Pearl Constellation V2 benchmark renderers while preserving the current 900-dot baseline

## Context

Pearl’s current dense point-cloud / pose-constellation figure is still the strongest visual direction.
...
- well suited to Pearl’s intended audience of adults around 45–65.

The goal is therefore **not** to replace the constellation aesthetic.

The goal is to refine it visually and rebuild its rendering architecture so Pearl can preserve the premium 900-dot appearance with much lower renderer overhead and latency.
...
1. **Constellation V2 · 900**
   ...
   - intended to prove whether Pearl can keep the preferred 900-dot richness without the current React Native SVG overhead.
...
Implement a benchmark-only Android native constellation overlay inside Pearl’s existing local Expo pose-detection module.
...
The existing JS event path must remain active so Pearl’s benchmark diagnostics and future measurement logic still receive the same pose results.
...
They may sit around a restrained torso measurement region or Pearl signature cluster.
...
> Can Pearl retain the full premium 900-dot appearance while removing most renderer overhead?
