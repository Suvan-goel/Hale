# Hale Movement Check-Up Physical-Device Bug Audit
...
- `/tmp/hale_balance_probe.ts`
- `/tmp/hale_warden_age19_probe.ts`
...
Deterministic replay results from `/tmp/hale_balance_probe.ts`:
...
Deterministic replay from `/tmp/hale_balance_probe.ts` simulated a valid hold, then foot-down at about 20 seconds.
...
- Current hands-free behavior gives the opposite impression: "Rest before the next attempt. Hale will continue automatically" but then saves best and advances after default rest.
...
The result is surprising mainly because Hale is intended for adults about 45-65 while the product owner tested with exact age 19. The Warden source supports age 18+, so the current app is mathematically allowed to compare a 19-year-old.
...
- Clarify whether "Hale will continue automatically" means next attempt or best saved.
