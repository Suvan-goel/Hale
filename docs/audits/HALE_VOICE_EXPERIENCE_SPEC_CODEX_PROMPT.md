# Codex Prompt: Hale Voice Experience Specification and Final Script Design

Read this entire prompt before starting.

Hale has now completed two evidence-backed voice audits:
...
Create the definitive Hale Voice Experience Specification for:
...
1. `docs/specs/HALE_VOICE_EXPERIENCE_SPEC.md`
2. `docs/specs/HALE_VOICE_EXPERIENCE_SPEC.json`
3. `docs/specs/HALE_VOICE_SCRIPT_REVIEW.md`
4. `docs/specs/HALE_VOICE_SCRIPT_MANIFEST.csv`

Do not overwrite the audit files.

## Purpose of each artifact

### `HALE_VOICE_EXPERIENCE_SPEC.md`
...
### `HALE_VOICE_EXPERIENCE_SPEC.json`
...
### `HALE_VOICE_SCRIPT_MANIFEST.csv`
...
# 5. Hale product principles

The final voice experience must reflect these product principles.

## Audience

Hale is primarily for adults aged approximately 45–65 who want to remain strong, steady, mobile, and independent.
...
A user should be able to complete the core flow without staring at the screen, but Hale should not talk constantly.
...
Do not add spoken rep-by-rep counting unless there is a clear product reason. Hale should not become noisy.
...
Hale uses bundled audio, not runtime TTS.
...
`HALE_VOICE_SCRIPT_REVIEW.md` must be easy to review without reading the engineering specification.

Use this structure:

# Hale Voice Script Review
