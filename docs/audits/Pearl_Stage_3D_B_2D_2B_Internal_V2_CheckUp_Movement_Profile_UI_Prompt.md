You are implementing Stage 3D-B.2D.2B of Pearl’s production-readiness work:
...
- Display the frozen Pearl task band:
...
```text
Your raw results are already saved. These optional details help Pearl choose the closest published reference group.
```
...
- display calm recovery:
  - `This result is already saved. Pearl will use the saved version.`
...
```text
Pearl saved your raw result. A published chair-rise comparison is not available for this result yet.
```
