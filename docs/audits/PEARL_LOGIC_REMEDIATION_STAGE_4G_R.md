# Pearl Logic Remediation Stage 4G-R
...
?? docs/audits/PEARL_POSE_OVERLAY_PHYSICAL_DEVICE_BENCHMARK_PROMPT.txt
?? docs/audits/Pearl_Stage_4F_R_1_Progression_Model_Enforcement_Prompt.md
?? docs/audits/Pearl_Stage_4G_R_Mobility_Rotation_Equipment_Positioning_Prompt.md
...
```text
 ...
 src/pearlFlow/__tests__/exploreViewModel.test.ts    |  52 +-
 src/pearlFlow/__tests__/progressionEvidence.test.ts |  71 ++-
 src/pearlFlow/__tests__/sessionPlanning.test.ts     | 123 ++--
 src/pearlFlow/exploreViewModel.ts                   |  37 +-
 src/pearlFlow/progressionEvidence.ts                | 106 +++-
 src/pearlFlow/sessionPlanning.ts                    | 220 ++++++-
 src/pearlFlow/types.ts                              |   9 +
...
Added `src/pearlFlow/equipmentPositioning.ts` as the canonical controlled-beta equipment positioning source:
...
- Pearl plans around what the user has, but some movements may be unavailable without household support or a resistance band.
...
- `src/pearlFlow/equipmentPositioning.ts`
...
```text
npm test -- --runInBand src/training/__tests__/collectionSelection.test.ts src/training/__tests__/workoutGeneration.test.ts src/training/__tests__/store.test.ts src/pearlFlow/__tests__/exploreViewModel.test.ts src/pearlFlow/__tests__/copyGuardrails.test.ts src/pearlFlow/__tests__/sessionPlanning.test.ts src/exercises/__tests__/progressionPolicy.test.ts src/exercises/__tests__/catalog.test.ts src/services/backend/__tests__/restoreService.test.ts src/services/backend/__tests__/trainingStateSyncService.test.ts src/pearlFlow/__tests__/stage5hLifecycle.integration.test.ts
...
```text
rm -rf /tmp/pearl-stage4gr-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-stage4gr-export
rc=$?
rm -rf /tmp/pearl-stage4gr-export
exit $rc
PASS
Note: export printed the existing Sentry config warning and repeated NO_COLOR/FORCE_COLOR Node warnings. Android and iOS bundles exported successfully, and /tmp/pearl-stage4gr-export was removed.
```
