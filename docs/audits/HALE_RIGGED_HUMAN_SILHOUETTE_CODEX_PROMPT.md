...
Hale has a diagnostics-only pose-overlay benchmark screen used to compare renderer designs and measure their effect on latency.
...
Create an original, gender-neutral adult silhouette for Hale.
...
The result must look like an original, continuous, filled human silhouette similar in overall coherence to the user’s reference image, while being internally rigged to the MediaPipe skeleton and remaining fast enough for Hale’s live benchmark.
