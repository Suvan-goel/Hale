...
Pearl currently uses a MediaPipe Pose Landmarker Full pipeline and has a diagnostics-only pose-overlay benchmark screen.
...
A previous benchmark-only “sculpted figure” was implemented from simple body pieces. It looks cheap, toy-like and mannequin-like. It is not suitable for Pearl and should be replaced rather than refined.
...
Use one very small, restrained Pearl detail, such as:
...
- No self-intersection artefacts during common Pearl movements.
...
```text
docs/audits/PEARL_MATTE_GRAPHITE_DIGITAL_TWIN_BENCHMARK.md
```
