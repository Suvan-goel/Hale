You are implementing a narrow Progress page product/UI restoration pass for Hale:
...
```bash
rm -rf /tmp/hale-progress-restoration-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/hale-progress-restoration-baseline-export
rc=$?
rm -rf /tmp/hale-progress-restoration-baseline-export
exit $rc
```
...
- Balance can show a Hale task band in plain language.
...
```text
Frozen from Jun 26, 2026. Hale shows saved raw results and saved reference labels only.
```
...
```text
What you are practicing now
Hale adjusts these movements based on your completed sessions.
```
...
```bash
rm -rf /tmp/hale-progress-restoration-export
npx --no-install expo export --platform all --output-dir /tmp/hale-progress-restoration-export
rc=$?
rm -rf /tmp/hale-progress-restoration-export
exit $rc
```
