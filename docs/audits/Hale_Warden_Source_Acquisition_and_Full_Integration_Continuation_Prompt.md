You are carrying out Hale’s Warden source-acquisition and integration continuation:

HALE WARDEN SOURCE ACQUISITION + FULL INTEGRATION CONTINUATION
...
```text
Hale has written permission from the Warden study authors to use the approved 30-second chair-stand percentile data/calculator/formula in the app.
```
...
If official supplementary links require authentication or cannot be downloaded programmatically, record that and stop with instructions for the product owner to manually place the official files under `/tmp/hale-warden-source/`.
...
- For `.zip`: unzip to `/tmp/hale-warden-source/extracted/`.
...
```text
docs/sources/HALE_WARDEN_SOURCE_IMPLEMENTATION_NOTES.md
```
...
10th to <25th percentile:
  hale_starting_point / Strength candidate

25th to <40th percentile:
  hale_building / Strength candidate if no clearer lower domain
...
```bash
rm -rf /tmp/hale-warden-source-acquisition-integration-export
