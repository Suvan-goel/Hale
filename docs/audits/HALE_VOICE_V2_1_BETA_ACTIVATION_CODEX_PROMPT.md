# Codex Prompt: Hale Voice V2.1 Full Beta Activation With Fast Legacy Rollback

Read this entire prompt before changing anything.

The founder wants to enable the new Voice V2.1 system throughout Hale for real-device and beta-tester feedback.
...
The label can be refined to fit Hale’s UI style.
...
New voice system
Use Hale’s latest voice guidance with clearer setup, countdowns, and recovery prompts.
...
```bash
find assets/audio -type f | sort > /tmp/hale_voice_v21_beta_activation_entry.files
find assets/audio -type f | sort | xargs shasum -a 256 > /tmp/hale_voice_v21_beta_activation_entry.sha256
```

At task end:

```bash
find assets/audio -type f | sort > /tmp/hale_voice_v21_beta_activation_exit.files
find assets/audio -type f | sort | xargs shasum -a 256 > /tmp/hale_voice_v21_beta_activation_exit.sha256
diff -u /tmp/hale_voice_v21_beta_activation_entry.files /tmp/hale_voice_v21_beta_activation_exit.files
diff -u /tmp/hale_voice_v21_beta_activation_entry.sha256 /tmp/hale_voice_v21_beta_activation_exit.sha256
```
...
1. `docs/audits/HALE_VOICE_V2_1_BETA_ACTIVATION_IMPLEMENTATION.md`
2. `docs/audits/HALE_VOICE_V2_1_BETA_ACTIVATION_AUDIT.md`
3. `docs/audits/HALE_VOICE_V2_1_BETA_ACTIVATION_AUDIT.json`
4. `docs/audits/HALE_VOICE_V2_1_BETA_ACTIVATION_SCENARIOS.csv`
5. `docs/audits/HALE_VOICE_V2_1_BETA_ACTIVATION_READINESS_MATRIX.csv`
6. `docs/audits/HALE_VOICE_V2_1_BETA_ACTIVATION_ROLLBACK_PLAN.md`
7. `docs/audits/HALE_VOICE_PROJECT_POST_BETA_ACTIVATION_HANDOFF.md`
...
# Hale Voice V2.1 Beta Activation Implementation
...
```text
docs/audits/HALE_VOICE_PROJECT_POST_BETA_ACTIVATION_HANDOFF.md
```
