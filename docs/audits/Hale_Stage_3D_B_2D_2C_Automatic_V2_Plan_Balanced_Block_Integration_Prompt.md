...
The user explicitly starts a training session later, but Hale owns the personalisation and plan-creation step automatically.
...
It must integrate those frozen V2 artifacts with Hale’s already-verified Stage 5 training lifecycle.
...
```text
HALE_MOVEMENT_PROFILE_V2_PHYSICAL_DEVICE_VALIDATION.md
HALE_LOGIC_VERIFICATION_STAGE_3D_B_2E_B.md
HALE_MOVEMENT_PROFILE_V2_DEVICE_VALIDATION_REPORT.md
```
...
- Balance task bands may drive Hale-band focus.
...
- `Hale prepared this plan from your Movement Profile.`
...
- `Build my plan.`
- `Hale is building your plan` as a required user action.
- `Weakest domain.`
...
```bash
rm -rf /tmp/hale-stage3db2d2c-export
npx --no-install expo export --platform all --output-dir /tmp/hale-stage3db2d2c-export
rc=$?
rm -rf /tmp/hale-stage3db2d2c-export
exit $rc
```
