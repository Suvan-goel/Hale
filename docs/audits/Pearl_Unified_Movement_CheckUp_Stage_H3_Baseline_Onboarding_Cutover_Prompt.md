...
PEARL UNIFIED MOVEMENT CHECK-UP — STAGE H3
...
Its purpose is to make the existing polished unified Check-Up and unified Movement Profile flow available through Pearl’s normal baseline/onboarding journey when an explicit build-time release flag is enabled.
...
```bash
rm -rf /tmp/pearl-unified-h3-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-unified-h3-baseline-export
rc=$?
rm -rf /tmp/pearl-unified-h3-baseline-export
exit $rc
```
