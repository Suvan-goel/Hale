# HALE_PERCENTILE_SCORING_PROTOCOL_DESIGN_STAGE_3D_B_1
...
Hale should replace the controlled-beta headline of exact "movement age" with a `Movement Profile` made of raw metrics, source-backed reference comparisons, conservative claim suppression, and a transparent `Suggested focus`.
...
4. Balance: 45-second one-leg hold protocol, adaptive best-of-three, raw seconds plus Hale task band headline; source age-group comparison in detail, no exact percentile.
...
```text
 ...
 M src/haleFlow/__tests__/assessmentResultState.test.ts
 ...
 M src/haleFlow/extraSessionCopy.ts
...
?? docs/audits/Hale_Stage_3D_B_1_Percentile_Protocol_Focus_Design_Prompt.md
...
src/training/workoutGeneration.ts
 ...
 src/haleFlow/__tests__/planViewModel.test.ts       |  17 +-
 src/haleFlow/__tests__/progressViewModel.test.ts   |  14 +-
 src/haleFlow/appLifecycle.ts                       |  30 +-
 src/haleFlow/assessmentResultState.ts              |   8 +-
 src/haleFlow/copy.ts                               |  84 +++---
 src/haleFlow/extraSessionCopy.ts                   |   2 +-
 src/haleFlow/planViewModel.ts                      |  58 ++--
 src/haleFlow/progressViewModel.ts                  |  63 ++--
 src/haleFlow/sessionPlanning.ts                    |   7 +-
 src/haleFlow/types.ts                              |   1 +
...
- Watchman recrawl warning for `/Users/suvangoel/Hale`.
...
- `src/haleFlow/assessmentEligibility.ts` requires current snapshots and all headline domains for official block creation.
- `src/haleFlow/reports.ts` already suppresses direct comparison when score snapshot versions are incompatible.
...
- Allowed as product interpretation when labelled as Hale's task band rather than a population norm.
...
| Domain | Raw | Exact percentile | Percentile range | Age-group reference | Hale task band | Suggested-focus input |
| --- | --- | --- | --- | --- | --- | --- |
...
| `raw_only_reference_unavailable` | Source transform unavailable, outside source age, unsupported reference group | Raw plus Hale band | Source-relative claim | Add profile data or wait for source approval | Limited | source unavailable reason |
| `raw_only_profile_incomplete` | Missing exact age or reference sex where required | Raw plus Hale band | Reference claim | Add optional reference inputs | Limited | missing input |
...
- `/tmp/hale-stage3db1/warden_calculator.xlsx` from OUP supplementary appendix.
- `/tmp/hale-stage3db1/warden_pzab299.pdf` attempted download resolved to HTML rather than PDF and was not used as evidence.
...
| Question | Answer |
| --- | --- |
| Can Hale compute exact percentiles locally from published parameters? | Technically likely yes, because the official workbook contains formulas/hidden data sufficient to compute outputs. |
| Can Hale lawfully reproduce the required parameters? | Not yet approved. The source is accessible, but the calculator copyright statement requires legal/product approval before embedding extracted tables/formulas in app code. |
...
- Recommendation for Hale: do not count final partial reps in camera scoring until the rep tracker can reliably classify halfway-at-expiry. If future implementation supports that rule with tests, it may count a halfway final rise only when the threshold is deterministic.
...
| --- | --- | --- | --- |
| Norm eligible | `Chair-rise capacity` / `16 rises in 30 seconds` | `Around the 40th-60th percentile for your reference group` | `Based on a similar published test setup. Hale's camera estimate is not a medical assessment.` |
...
| Hand push-off detected | `Chair-rise capacity` / `16 rises in 30 seconds` | `Saved without a reference comparison` | `This result used extra arm support, so Hale will not compare it with the published chair-rise setup.` |
...
| Outside source age | `Chair-rise capacity` / `16 rises in 30 seconds` | `Saved as your personal baseline` | `The published reference model Hale uses does not cover your age.` |
...
| Low result | `Chair-rise capacity` / `7 rises in 30 seconds` | `Below the 10th percentile for your reference group` | `Hale can use this as a suggested focus, but it is not a diagnosis.` |
...
- Hale task band: allowed and recommended.
- Current 12-second Hale ladder: incompatible with this source because users can hit the current ceiling far below the source 45-second ceiling.
...
- Headline: raw seconds plus Hale task band.
...
| State | Card headline | Secondary text | Detail/caveat |
| --- | --- | --- | --- |
| Full 45 seconds | `One-leg balance` / `45 seconds` | `Full 45-second hold completed` | `This test stops at 45 seconds, so Hale records completion rather than a higher score.` |
...
| Fewer than required valid trials | `One-leg balance` / `18 seconds` | `Saved as a personal baseline` | `Hale needs the planned valid attempts before showing a published comparison.` |
...
- Exact longitudinal change interpretation: blocked pending Hale device repeatability.
...
| Above IQR | `Active shoulder reach` / `178 deg` | `Above the published middle range` | `More range is not automatically better; Hale records this as a reference comparison only.` |
...
| Pain-limited | `Active shoulder reach` / `120 deg` | `Saved without a reference comparison` | `Hale does not compare results that were limited by discomfort.` |
...
- If user gives only Hale's old age band, suppress exact chair percentile range and show raw-only until exact age is supplied.
- Birthdays: freeze age-at-test in snapshot. Do not recompute old results after birthday.
- Outside source range: raw-only and Hale task band; no extrapolated reference claims.
- Users who prefer not to say: raw-only and Hale task band.
...
| Balance | product-created Hale task band plus source age-group benchmark | Springer best-of-three eyes-open table; no percentile |
...
- Balance: first controlled beta should avoid `within reference range` unless the dispersion field is source-reviewed. Use source benchmark detail or Hale task band.
...
| Outside source age | Raw-only plus Hale task band |
...
3. If no domain is below reference but one domain is `hale_starting_point`, suggest it with raw/Hale provenance.
...
| One domain raw-only | Use eligible domains; raw-only can contribute only as `hale_starting_point` if raw band is low |
...
- At least two domains have eligible reference or Hale task categories, and
...
| `checkup_hale_band_focus` | Raw/Hale category clearly low, no source claim | `Suggested focus from today's task results` |
...
- Reports across old/new policy show: `Hale's scoring method has changed since your earlier Check-Up, so a direct comparison isn't available.`
...
- Use `reference group`, `published comparison`, `published middle range`, `Hale task band`.
...
| Balance | `28 seconds` | source benchmark detail/Hale band | raw/Hale band only | invalid trial retry | fewer valid attempts -> raw-only | raw/Hale band | raw-only | retake |
...
- Raw-only valid: limited; can support goal-led or Hale-band focus.
...
- participant explanation of Hale task band,
...
- User thinks Hale diagnosed weakness, fall risk, disease, or impairment.
...
| Stage 3D-B.2C snapshot/version/sync migration | version names, backend payload compact shape | scoreSnapshot, haleFlow assessments, backend sync/restore | round-trip, legacy fail-closed, restore | source fingerprints | no | yes |
...
| Balance claim | Raw + Hale band; source benchmark detail | No percentile model | Low | Moderate | New display kinds | yes | yes |
...
3. Chair final-partial rule: pure full-stand count versus source half-up-at-expiry rule. The source uses half-up; Hale's current rep tracker should not claim it until deterministic.
...
```bash
rm -rf /tmp/hale-stage3db1 && test ! -e /tmp/hale-stage3db1 && echo '/tmp/hale-stage3db1 removed'
```

Output:

```text
/tmp/hale-stage3db1 removed
```
