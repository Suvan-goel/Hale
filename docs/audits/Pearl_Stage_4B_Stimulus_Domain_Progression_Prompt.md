You are implementing Stage 4B of Pearl’s production-readiness work:
...
- Upper-body pull no longer silently degrades into shoulder mobility; if no band is available, Pearl skips the upper-pull slot honestly.
...
D. Recommend the Pearl Movement Kit / resistance band for upper-back work.
...
- Add clear session guidance:
  - “Upper-back pulling needs a resistance band, so Pearl skipped that slot rather than replacing it with a different movement.”
...
- “Balance holds need nearby support, so Pearl used a safer supporting option today.”
- “A stair drill needs both a stable step and support nearby, so Pearl used chair-rise work instead.”
