You are implementing Stage 3D-B.2D.1 of Hale’s production-readiness work:
...
Before screens or blocks consume V2 results, Hale needs one deterministic contract answering:

> Given an immutable official V2 snapshot, the user’s explicitly selected life goal, and optional prior V2 focus context, what should Hale describe as the suggested focus—or should it use a balanced plan?
...
```bash
rm -rf /tmp/hale-stage3db2d1-export
npx --no-install expo export --platform all --output-dir /tmp/hale-stage3db2d1-export
rc=$?
rm -rf /tmp/hale-stage3db2d1-export
exit $rc
```
...
- task-band evidence source is `hale_task_band`, not published reference;
...
- retained raw-only balance with a valid task band may still provide Hale-band focus evidence only when the raw measurement is valid and not pain-limited;
...
### Rule 2 — Hale starting-point candidates

Only when there are no `below_reference` candidates.

Build domains with category `hale_starting_point`.
...
- Hale-band focus mode requires Hale starting-point origin;
...
- one Hale starting-point;
- multiple Hale starting-point + unique goal;
- multiple Hale starting-point + no goal -> balanced;
...
10. Balance starting-point bands may drive Hale-band focus.
...
17. One Hale starting-point domain wins only when no below-reference domain exists.
