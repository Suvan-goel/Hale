You are carrying out a narrow repository health-gate remediation for Pearl:

PEARL TRAINING RUNTIME HEALTH GATE
...
```bash
rm -rf /tmp/pearl-step-up-resume-health-gate-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-step-up-resume-health-gate-export
rc=$?
rm -rf /tmp/pearl-step-up-resume-health-gate-export
exit $rc
```
