# Pearl Logic Audit Stage 3D-B: Norm Provenance and Source Verification
...
- No claim was made that Pearl is beta-ready.
...
Temporary source downloads and render artifacts were kept under `/tmp/pearl-stage3db/` and were removed before final handoff.
...
?? docs/audits/Pearl_Stage_3D_B_Norm_Provenance_Source_Verification_Prompt.md
```
...
```bash
npm test -- --runInBand src/scoring/__tests__/scoringInputValidation.test.ts src/scoring/__tests__/scoring.test.ts src/scoring/__tests__/scoreSnapshot.test.ts src/scoring/__tests__/focusSelection.test.ts src/pearlFlow/__tests__/assessmentEligibility.test.ts src/pearlFlow/__tests__/checkupHistory.test.ts src/pearlFlow/__tests__/progressViewModel.test.ts src/pearlFlow/__tests__/reports.test.ts src/pearlFlow/__tests__/copyGuardrails.test.ts src/services/backend/__tests__/checkupSyncService.test.ts src/services/backend/__tests__/restoreService.test.ts src/exercises/__tests__/catalog.test.ts src/pearlFlow/__tests__/stage5hLifecycle.integration.test.ts
```

Result: 12 suites passed, 145 tests passed. Note: `src/pearlFlow/__tests__/reports.test.ts` did not match an existing test file, so report coverage was verified by the next targeted command.

Targeted report/flow validation command:

```bash
npm test -- --runInBand src/pearlFlow/__tests__/pearlFlow.test.ts
```
...
| --- | ---: | ---: | --- | --- |
| Chair stand reps | 47 | 19.00 reps | true | Pearl extrapolation, unverified |
| Chair stand reps | 52 | 18.00 reps | true | Pearl extrapolation, unverified |
| Chair stand reps | 57 | 17.00 reps | true | Pearl extrapolation, unverified |
...
| TUG seconds | 50 | 7.00 sec | true | Pearl extrapolation, unverified |
| TUG seconds | 55 | 7.40 sec | true | Pearl extrapolation, unverified |
...
| Single-leg stance | 50 | 35.00 sec | true | Pearl extrapolation, unverified |
| Single-leg stance | 55 | 31.00 sec | true | Pearl extrapolation, unverified |
...
| Unsupported/current Pearl-estimated | 4 | Shoulder 50/60/70/80 |
...
| `src/pearlFlow/progressViewModel.ts` | Strength age range label retained; balance/mobility softened | Medium |
...
| `src/pearlFlow/reports.ts` and `BlockReportScreen.tsx` | Change/similar/new-data copy, focus suggested from latest snapshot | Medium |
...
5. Compare source protocols to Pearl movement protocols.
6. Compare source values to Pearl anchors and record transformations.
...
| B | Source identity, protocol, or metadata verified from primary/official source, but exact current Pearl table values were not directly accessible. |
...
| E | Unsupported, Pearl-estimated, extrapolated, or incompatible with verified source values. |
...
- The CDC 30-second chair-stand protocol matches the core movement used by Pearl: 17-inch chair, arms crossed, repeated full stands for 30 seconds.
- The author article by Jones/Rikli provides normal-range tables for men and women ages 60-94. Those values can reproduce Pearl's older-adult chair anchors if Pearl takes the midpoint of each male range, the midpoint of each female range, then averages those two sex-specific midpoints.
...
- No primary source was found for Pearl's ages 47, 52, and 57 anchors.
...
| Protocol element | CDC/SFT source | Pearl implementation | Verdict |
| --- | --- | --- | --- |
...
Protocol verdict: the movement construct is appropriate, but Pearl's source-to-product transformation is not fully documented because of sex pooling, under-60 extrapolation, and camera-specific counting differences.

## 13. Chair-Stand Anchor Verification

Author article normal-range values inspected:

| Age bin | Men normal range | Women normal range | Pearl direct-derived value | Calculation |
| --- | --- | --- | ---: | --- |
...
| Anchor | Pearl value | Audit grade | Verdict |
| ---: | ---: | --- | --- |
...
| Source age group | Source mean | Pearl anchor | Verification |
| --- | ---: | ---: | --- |
...
- Pearl's TUG is side-lateral camera framed, uses seat-off to re-seated timing, and detects a turn via hip-x velocity reversal.
- Pearl also supports a short-path non-standard variant, which should not be norm-compared as standard TUG.
...
| Protocol element | Bohannon source | Pearl implementation | Verdict |
| --- | --- | --- | --- |
| Eyes open | Yes | Single-leg eyes-open stage is the scored balance driver | Match |
| Trial cap | Source studies varied: 30s, 45s, 60s, no limit, or unstated | Pearl caps single-leg EO at 12s | Major mismatch |
...
| Trials | Varied | Pearl ladder stage once per check-up | Partial |
| Population | Ages 60-99 | Pearl target includes 45-65+ | Under-60 unsupported |

Protocol conclusion: the Pearl single-leg construct is source-adjacent, but the 12-second cap makes the current age mapping invalid for a public balance-age claim.

## 17. Balance-Cap Mismatch Analysis

BALANCE AGE CLAIM BLOCKED

The current Pearl balance stage caps single-leg eyes-open hold at 12 seconds:
...
- A perfect valid Pearl single-leg result cannot reach the 60-69 source mean.
- A perfect valid Pearl single-leg result cannot reach the 70-79 source mean.
- A perfect valid Pearl result maps to roughly age 81 before camera/device error.
...
The current code cites broad shoulder-ROM sources, especially Norkin & White-style goniometry references and aging ROM references, but it does not cite a precise table that reproduces Pearl's anchors:
...
| Source | What it supports | Does it verify current Pearl table? |
| --- | --- | --- |
| Norkin & White goniometry manual | Measurement method and clinical ROM context | No exact age-stratified Pearl values verified |
| CDC/Soucie ROM study | Official ROM reference values by broad age and sex groups through age 69 | No; age bins and values do not match Pearl table |
...
Gill et al. is particularly relevant because it contains age- and sex-stratified active shoulder flexion data in a general population cohort. It resembles the broad age decline implied by Pearl's table, but Pearl neither cites it nor exactly derives its anchors from it.

## 19. Shoulder Anchor Verification

| Anchor | Pearl value | Current code estimated flag | Audit grade | Verdict |
| ---: | ---: | --- | --- | --- |
...
| Balance | Single-leg source means but Pearl protocol caps at 12s | Major scale distortion |
...
| --- | --- | ---: | ---: | --- | --- |
| CS-47 | Strength | 47 | 19.00 reps | Pearl extrapolated | E |
| CS-52 | Strength | 52 | 18.00 reps | Pearl extrapolated | E |
| CS-57 | Strength | 57 | 17.00 reps | Pearl extrapolated | E |
...
| TUG-50 | Mobility/supporting | 50 | 7.00 sec | Pearl extrapolated | E |
| TUG-55 | Mobility/supporting | 55 | 7.40 sec | Pearl extrapolated | E |
...
| SLS-50 | Balance | 50 | 35.00 sec | Pearl extrapolated | E |
| SLS-55 | Balance | 55 | 31.00 sec | Pearl extrapolated | E |
...
| SF-50 | Mobility | 50 | 162.00 deg | Pearl estimated/unverified | E |
| SF-60 | Mobility | 60 | 158.00 deg | Pearl estimated/unverified | E |
| SF-70 | Mobility | 70 | 150.00 deg | Pearl estimated/unverified | E |
| SF-80 | Mobility | 80 | 140.00 deg | Pearl estimated/unverified | E |
...
| 80-99 to age 85 | TUG/SLS source group is 80-99; Pearl anchors at 85 | Not source-justified | Medium |
...
Risk: future score snapshots may be hard to interpret if norm tables or display policies change. This matters because Pearl's product promise is longitudinal trend comparison.
...
| Balance | Single-leg source values partially verified; Pearl cap mismatch critical | BALANCE AGE CLAIM BLOCKED | Raw capped hold seconds can display as home estimate | Blocked as calibrated focus input |
...
| Balance age | BLOCKED | Verified SLS source values are incompatible with Pearl's 12s scoring cap. |
...
- Decide whether Pearl will score a longer single-leg hold, use capped product bands only, or source a cap-compatible balance reference.
...
```text
 ...
 src/pearlFlow/__tests__/sessionPlanning.test.ts     | 15 ++++
 src/pearlFlow/sessionPlanning.ts                    | 26 ++++---
...
Final interpretation: the tracked dirty files at final status differ from the initial snapshot, which indicates worktree changes occurred outside this audit while the audit was in progress. This audit created only the untracked report file `docs/audits/PEARL_LOGIC_AUDIT_STAGE_3D_B.md`, did not edit production code, and did not stage or commit anything.
...
Cleanup status: confirmed removed with `rm -rf /tmp/pearl-stage3db && test ! -e /tmp/pearl-stage3db`.
