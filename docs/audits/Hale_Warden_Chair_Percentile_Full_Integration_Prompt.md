You are implementing Hale’s approved Warden chair-stand percentile integration:

HALE WARDEN CHAIR-STAND FULL INTEGRATION
...
The product owner has confirmed that Hale has written permission from the Warden study authors to use the relevant percentile data / calculator / formula in the app.

## Executive product decision

Implement Warden fully in Hale:
...
The product owner states Hale has all required written permission.
...
Scope: use of approved 30-second chair-stand percentile data/calculator/formula in Hale app.
...
```text
Which reference group should Hale use for published movement comparisons?
Female
Male
```
...
```text
Published movement reference data is usually grouped this way. Hale uses this only to compare your movement results with the right reference data.
```
...
```text
How old are you?
Hale uses your age to compare your check-up results with published movement reference data.
```
...
- outside Warden source age range -> raw-only Warden fallback but user can still use Hale.
...
10th to <25th percentile:
  hale_starting_point for Strength / Power.

25th to <40th percentile:
  hale_building for Strength / Power.
...
```bash
rm -rf /tmp/hale-warden-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/hale-warden-baseline-export
rc=$?
rm -rf /tmp/hale-warden-baseline-export
exit $rc
```
...
```text
Which reference group should Hale use?
Published movement reference data is usually grouped this way. Hale uses this only for movement comparisons.
```
...
```bash
rm -rf /tmp/hale-warden-full-integration-export
npx --no-install expo export --platform all --output-dir /tmp/hale-warden-full-integration-export
rc=$?
rm -rf /tmp/hale-warden-full-integration-export
exit $rc
```
