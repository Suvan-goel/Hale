# Codex Prompt: Hale Voice Runtime Timeline, Accuracy, and Collision Audit

Read this entire prompt before making any changes.

We have already completed a static inventory of Hale’s voice cues:

- `docs/audits/HALE_VOICE_CUE_INVENTORY.md`
- `docs/audits/HALE_VOICE_CUE_INVENTORY.json`

Those files identified 110 spoken cue templates and confirmed that Hale uses bundled MP3 files at runtime through `VoiceChannel` in `src/audio/voicePlayer.ts`, rather than runtime TTS.
...
Perform a second-stage, evidence-backed audit that reconstructs what a real Hale user would actually hear during:
...
- Whether Hale is genuinely usable as a voice-first, eyes-off-screen experience
...
1. `docs/audits/HALE_VOICE_RUNTIME_TIMELINE_AUDIT.md`
2. `docs/audits/HALE_VOICE_RUNTIME_TIMELINE_AUDIT.json`
3. `docs/audits/HALE_VOICE_ASSET_DURATIONS.csv`
...
Enumerate all physical MP3 files used by Hale’s voice system for both voices.
...
17. On resume, does Hale replay essential instructions or continue with missing context?
...
Hale’s spoken introduction says the user can follow the voice without touching the screen. Test that promise against actual information coverage.
...
# Hale Voice Runtime Timeline Audit
