You are carrying out a verification-first UX / product-logic audit for Hale:
...
That conflicts with Hale’s intended product experience:

```text
The user should place the phone down,
step back into view,
get into the correct position,
and Hale should start each movement automatically once camera readiness is detected.
```
...
```text
After the user taps Start Check-Up and places the phone, Hale should not require screen interaction to complete the Movement Check-Up.
...
Do not propose a fully invisible state. Users should always know what Hale is waiting for.
