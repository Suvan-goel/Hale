# Hale Unified Movement Check-Up - Stage H4.1.1 Policy-Aligned Closure Continuation
...
```text
npm test -- --runInBand src/training/stepUpAlternation/__tests__/stepUpRuntimeIntegration.test.ts src/training/__tests__/sessionPlayer.test.ts src/haleFlow/__tests__/sessionPlanning.test.ts src/services/backend/__tests__/trainingStateSyncService.test.ts src/services/backend/__tests__/restoreService.test.ts
```
...
4. If exactly one current domain is at the Hale starting point, choose that domain.
5. If multiple current domains are at the Hale starting point, preserve a prior official-retest
...
leave orphan V2 report/transition state in local Hale files. Existing account data tests
remain green.
...
Home lifecycle uses the same `getHaleAppLifecycle` source as Today/next-best-action behavior.
...
```text
npm test -- --runInBand src/haleFlow/__tests__/movementProfileV2OfficialRetestH411Continuation.integration.test.ts
...
```text
rm -rf /tmp/hale-unified-h411-continuation-export
npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h411-continuation-export
rc=$?
rm -rf /tmp/hale-unified-h411-continuation-export
exit $rc
passed
