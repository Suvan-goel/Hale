You are implementing Stage 3D-B.2B of Hale’s production-readiness work:
...
- Headline interpretation: Hale task band.
- No exact percentile.
- No Movement Age.
...
npx --no-install expo export --platform all --output-dir /tmp/hale-stage3db2b-export
rc=$?
rm -rf /tmp/hale-stage3db2b-export
exit $rc
```
...
- balance task band explicitly marked Hale-created product interpretation;
...
## Step 12: Implement the exact Hale balance task band
...
Do not guess a bin from an ambiguous broad Hale onboarding band.
