# HALE_WARDEN_CHAIR_PERCENTILE_FULL_INTEGRATION
...
This task attempted Hale's approved Warden chair-stand percentile full integration: exact age, reference sex, Warden 30-second chair-stand percentile transform, Strength / Power focus integration, plan creation influence, and consumer-safe Progress/results/report surfacing.
...
The product owner stated Hale has written permission from the Warden study authors to use the approved 30-second chair-stand percentile data/calculator/formula in the app.
...
?? docs/audits/Hale_Warden_Chair_Percentile_Full_Integration_Prompt.md
...
npx --no-install expo export --platform all --output-dir /tmp/hale-warden-full-integration-export: passed
```
