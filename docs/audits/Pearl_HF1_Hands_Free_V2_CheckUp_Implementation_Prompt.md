You are implementing the first hands-free remediation stage for Pearl:

PEARL HANDS-FREE FLOW — STAGE HF1
...
The founder tested the current V2 Movement Check-Up from onboarding and had to press buttons during the Check-Up. That is not acceptable for Pearl’s product promise.
...
Pearl detects correct position and stable readiness.
Pearl starts a voice countdown.
Movement starts automatically.
Movement ends automatically.
Pearl moves to the next movement.
...
After the user has entered the active Movement Check-Up flow and placed the phone, Pearl must not require screen interaction to complete the Check-Up.
...
```bash
rm -rf /tmp/pearl-hf1-baseline-export
...
The user should always know what Pearl is waiting for.
...
```bash
rm -rf /tmp/pearl-hf1-hands-free-checkup-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-hf1-hands-free-checkup-export
rc=$?
rm -rf /tmp/pearl-hf1-hands-free-checkup-export
exit $rc
```
