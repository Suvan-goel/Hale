# Hale Unified Movement Check-Up Stage H3.1 End-to-End Verification
...
git diff --check
npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h31-baseline-export
```
...
`mapRemoteHaleSnapshotToLocal` parses these artifacts without recomputing them. The restored local state contains:
...
```text
rm -rf /tmp/hale-unified-h31-export
npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h31-export
rc=$?
rm -rf /tmp/hale-unified-h31-export
exit $rc
```
