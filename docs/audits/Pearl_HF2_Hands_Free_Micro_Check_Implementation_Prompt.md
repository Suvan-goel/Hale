You are implementing the second hands-free remediation stage for Pearl:

PEARL HANDS-FREE FLOW — STAGE HF2
...
Pearl detects readiness and side/leg choice from camera/pose where needed.
Pearl starts the countdown automatically.
...
After the user starts a micro-check and places the phone, Pearl must not require screen interaction to complete the micro-check.
...
```bash
rm -rf /tmp/pearl-hf2-baseline-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-hf2-baseline-export
rc=$?
rm -rf /tmp/pearl-hf2-baseline-export
exit $rc
```
...
```bash
rm -rf /tmp/pearl-hf2-hands-free-micro-check-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-hf2-hands-free-micro-check-export
rc=$?
rm -rf /tmp/pearl-hf2-hands-free-micro-check-export
exit $rc
```
