...
Hale is implemented as a React Native/Expo app whose top-level runtime is coordinated by `App.tsx`. The app hydrates local file-backed stores, hydrates auth state, optionally restores remote Supabase state, computes a lifecycle state, and then routes into camera check-up, training, micro-check, onboarding, reports, settings, or tab screens. This is confirmed from runtime code in `AppGate` and `HaleApp` (`App.tsx:258-338`, `App.tsx:424-521`, `App.tsx:2051-2295`).
...
3. Dynamic planning can see non-training completions as recent sessions. `App.tsx` passes all `adherence.completions` into planning (`App.tsx:1406-1438`), `recentSessionsFor` maps them all to completed sessions (`src/haleFlow/sessionPlanning.ts:537-560`), and `getTemplateSelection` counts recent sessions by block (`src/training/workoutGeneration.ts:307-347`).
4. Equipment state is split between safety profile and legacy training equipment, with later generation applying both and deleting optional equipment from the safety-derived set (`App.tsx:1741-1781`, `src/haleFlow/sessionPlanning.ts:646-671`).
5. Account sign-out and local-data deletion are separate flows. Normal sign-out does not clear local Hale files (`src/components/AccountAuthCard.tsx:173-182`, `src/services/backend/AuthProvider.tsx:271-284`), while explicit clearing is a separate confirmed action (`src/components/AccountAuthCard.tsx:217-251`, `src/services/backend/accountDataService.ts:53-107`).
...
| --- | --- |
| Lifecycle | `src/haleFlow/appLifecycle.ts`, `src/adherence/dateUtils.ts`, `src/adherence/adherenceState.ts` |
...
| Scoring | `src/scoring/scoring.ts`, `src/scoring/norms.ts`, `src/haleFlow/assessments.ts` |
| Exercise catalogue | `src/exercises/index.ts`, `src/exercises/registry.ts`, `src/exercises/ladders.ts`, `src/exercises/*.ts` |
| Workout generation | `src/training/workoutGeneration.ts`, `src/haleFlow/sessionPlanning.ts`, `src/training/block.ts` |
...
| Progression | `src/training/progression.ts`, `src/training/workoutGeneration.ts`, `src/training/validTimeProgression.ts`, `src/haleFlow/sessionPlanning.ts` |
...
| Supabase auth storage | `src/lib/supabase.ts:23-31` | AsyncStorage on native | Confirmed from runtime code. Auth session is separate from Hale file stores. |
...
flowchart TD
  ...
  B -->|signed in| D["HaleApp local-store hydration"]
...
| --- | --- | --- | --- | --- | --- | --- |
| Launch -> auth gate | App mount | `loading`, `isSignedIn`, `isPasswordRecovery` | none | `AuthScreen` or `HaleApp` | `App.tsx:258-278` | Confirmed from runtime code |
| HaleApp hydration | `useEffect` on mount | camera permission, audio config, four local stores | local React state | `localStoresReady` true | `App.tsx:339-369` | Confirmed from runtime code |
...
| Lifecycle route | each render | profile, history, training, adherence, active block, date | none | `HaleAppLifecycleResult` | `App.tsx:1165-1175`, `src/haleFlow/appLifecycle.ts:97-147` | Confirmed from runtime code |
...
- Invalid official assessment can exist and be selected by `latestOfficialAssessment` because it filters by official type but not `status === 'completed'` (`src/haleFlow/assessments.ts:46-52`). Lifecycle's `hasOfficialAssessment` does filter completed status (`src/haleFlow/appLifecycle.ts:356-358`). Label: contradictory.
...
- Normal sign-out does not clear local Hale files. A separate destructive flow does (`src/components/AccountAuthCard.tsx:217-251`). Label: requires product decision.
...
| L02 | Local hydration | app launch | `HaleApp` load effect (`App.tsx:339-369`) | local stores | defaults on failure | React state | ready app state | store serializer tests | Lost/stale data | P0 |
...
| L04 | Lifecycle routing | Today/Plan | `getHaleAppLifecycle` (`src/haleFlow/appLifecycle.ts:97-147`) | profile/history/adherence/training/date | next state/action | none | lifecycle result | `appLifecycle.test.ts` | Wrong next action | P1 |
...
| L09 | Scoring/results | results/progress | `scoreCheckUp`, `createMovementAssessment` (`src/scoring/scoring.ts:69-206`, `src/haleFlow/assessments.ts:5-76`) | CheckUp | domain ages/weakest | assessments | score, focus | scoring tests | Wrong plan focus | P0 |
...
| L12 | Dynamic planning | Today/Plan/Explore | `planTodayHaleSession`, `generateTodaySession` (`src/haleFlow/sessionPlanning.ts:94-191`, `src/training/workoutGeneration.ts:357-443`) | block, equipment, pain, readiness, progress | session/exercises | active plan memory | preview plan | sessionPlanning/workoutGeneration tests | Wrong session | P0/P1 |
...
| L15 | Completion/progression | session complete/feedback | `handleSessionComplete`, `updateExerciseProgressionFromSession` (`App.tsx:1497-1590`, `src/haleFlow/sessionPlanning.ts:424-456`) | result, feedback, plan | counts/progress | training/adherence | completion, ladder progress | progression/workout tests | Plan advances wrongly | P0 |
...
- Authoritative entry point: `AppGate` and `HaleApp` (`App.tsx:258-338`).
...
- Unresolved assumptions: whether ordinary sign-out should preserve local Hale data.
...
- Preconditions: life goal and safety profile are required before baseline according to lifecycle (`src/haleFlow/appLifecycle.ts:360-362`).
...
- Authoritative entry points: `scoreCheckUp` (`src/scoring/scoring.ts:192-206`) and `createMovementAssessment` (`src/haleFlow/assessments.ts:5-76`).
...
- Preconditions: generated exercise IDs must exist in registry (`src/haleFlow/sessionPlanning.ts:125-135`, `src/haleFlow/sessionPlanning.ts:602-605`).
- Outputs: `HaleSessionPlan`.
...
- Failure modes: empty generated sessions fall back to legacy plan (`src/haleFlow/sessionPlanning.ts:125-143`); unsupported generated IDs fall back.
- Recovery behavior: legacy fallback uses `training.nextSessionExercises` (`src/haleFlow/sessionPlanning.ts:501-535`).
...
- `beginCheckUp` defaults to `manual_extra` if any latest assessment exists, otherwise `baseline`; official types are baseline, baseline retake, and official retest (`App.tsx:1183-1198`, `src/haleFlow/assessments.ts:42-44`). Confirmed from runtime code.
...
- Weekly micro-check is separate from full check-up and is not part of `DEFAULT_BATTERY` (`src/training/microCheck.ts:50-66`, `src/haleFlow/microCheck.ts:5-20`). Confirmed from runtime code.
...
- An invalid official assessment can be latest official in one helper but not count as baseline in lifecycle (`src/haleFlow/assessments.ts:46-52`, `src/haleFlow/appLifecycle.ts:356-358`). Label: contradictory.
...
- Equipment support is duplicated in `workoutGeneration.ts`, `sessionPlanning.ts`, and `exploreViewModel.ts` (`src/training/workoutGeneration.ts:964-982`, `src/haleFlow/sessionPlanning.ts:673-687`, `src/haleFlow/exploreViewModel.ts:235-260`). Label: P3 maintainability plus P1 correctness risk if they diverge.
...
2. `createMovementAssessment` maps score to `MovementAssessment` (`src/haleFlow/assessments.ts:5-76`).
...
5. Dynamic `planTodayHaleSession` converts `MovementBlock` to dynamic block/templates at session-planning time (`src/haleFlow/sessionPlanning.ts:94-191`, `src/haleFlow/sessionPlanning.ts:689-707`).
...
1. `planTodayHaleSession` derives session type/readiness/pain/preset (`src/haleFlow/sessionPlanning.ts:94-100`, `src/haleFlow/sessionPlanning.ts:607-644`).
2. It resolves available equipment from safety profile plus legacy equipment flags (`src/haleFlow/sessionPlanning.ts:646-671`).
...
4. If dynamic session is empty/unsupported, session planning falls back to legacy next-session IDs (`src/haleFlow/sessionPlanning.ts:125-143`, `src/haleFlow/sessionPlanning.ts:501-535`).
...
- Date input is not fully respected in `plannedDateKey`, which uses `new Date()` (`src/haleFlow/sessionPlanning.ts:856-858`). Label: software correctness risk for tests/backdated planning.

Failure and fallback paths:

- No active movement block or dynamic failure produces legacy fallback plan (`src/haleFlow/sessionPlanning.ts:186-191`, `src/haleFlow/sessionPlanning.ts:501-535`).
...
| Extra/preset/manual session | `countsTowardMainPlan` false for preset/manual and retest_prep | `src/haleFlow/sessionPlanning.ts:282-287` | Confirmed |
| Null session plan | `countsTowardMainPlan(null)` returns true | `src/haleFlow/sessionPlanning.ts:282-287` | Requires audit |
...
- Dynamic ladder progression updates in `handleSessionFeedback` by calling `updateExerciseProgressionFromSession` only when `countsTowardMainPlan` is true (`App.tsx:1689-1705`, `src/haleFlow/sessionPlanning.ts:424-456`). Confirmed from runtime code.
- `completedExerciseResultsForPlan` treats missing item as completed unless override says otherwise (`src/haleFlow/sessionPlanning.ts:568-600`). Requires audit.
...
- Lifecycle shows weekly micro-check when sessions this week are more than 0 and less than target and no micro-check completed this week (`src/haleFlow/appLifecycle.ts:337-344`). Confirmed from runtime code.
- Micro-check type follows active block focus: strength -> chair power, balance -> single-leg, mobility -> mobility reach (`src/haleFlow/microCheck.ts:5-20`). Confirmed.
...
- Session planning currently receives all adherence completions and can map micro-check completions into recentSessions (`App.tsx:1406-1438`, `src/haleFlow/sessionPlanning.ts:537-560`). Label: software correctness risk.

### 10.4 Monthly re-test and reports

- Retest due logic requires active block completed, completed sessions >= total, elapsed >= 28 days, due date today, or legacy block complete (`src/haleFlow/appLifecycle.ts:308-322`). Confirmed.
...
- `getRetestDueSummary` can show retest due when active block completed or due date is today/past (`src/haleFlow/progressViewModel.ts:167-205`). Confirmed.
...
- Lifecycle clean-slate prompt uses 14 inactive days based on active block completion/start or legacy last session (`src/haleFlow/appLifecycle.ts:324-335`). Confirmed.
- Restart sessions are generated via preset gentle restart or lower readiness (`src/haleFlow/sessionPlanning.ts:607-644`). Confirmed.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Auth session | AsyncStorage via Supabase | Supabase auth | Supabase user id | Supabase client/auth actions | `AuthProvider`, sync services | Supabase auth | session persists locally | local Hale files are independent of auth |
...
| DR-005 | Official progress types baseline/baseline_retake/official_retest | `App.tsx:1183-1198`, `src/haleFlow/assessments.ts:42-44` | Duplicated; partial | Manual check affects plan | Product |
| DR-006 | Lifecycle order: onboarding, baseline, block, retest, restart, first session, week complete, micro-check, normal | `src/haleFlow/appLifecycle.ts:97-147` | Centralized; direct | Wrong next action | Product |
| DR-007 | Weekly micro-check due when sessions this week >0 and < target and no micro-check | `src/haleFlow/appLifecycle.ts:337-344` | Centralized; direct | Too many/missing micro-checks | Product |
| DR-008 | Retest prompt gates: active block completed, sessions >= total, days >=28, due date today, legacy complete | `src/haleFlow/appLifecycle.ts:308-322` | Centralized-ish; direct | Early/late retest | Product |
| DR-009 | Inactive restart after 14 days | `src/haleFlow/appLifecycle.ts:324-335`, `src/adherence/adherenceState.ts:12-34` | Duplicated; direct | Wrong restart/lapse | Product |
...
| DR-050 | Assessment confidence high/medium/low by measured domain count | `src/haleFlow/assessments.ts:68-76` | Centralized; partial | invalid assessment trusted | Product |
...
| DR-061 | Default equipment chair+wall | `src/training/workoutGeneration.ts:1113-1117`, `src/haleFlow/sessionPlanning.ts:646-671` | Duplicated; direct | unsafe assumption | Safety/product |
...
| DR-071 | `countsTowardMainPlan`: retest_prep/preset/manual false, null true | `src/haleFlow/sessionPlanning.ts:282-287` | Centralized; direct | wrong plan advancement | Product |
| DR-072 | Dynamic progression only for dynamic plan and feedback path | `App.tsx:1689-1705`, `src/haleFlow/sessionPlanning.ts:424-456` | Split; partial | no progression after no feedback | Product |
| DR-073 | Completed exercise default true if missing result | `src/haleFlow/sessionPlanning.ts:568-600` | Centralized; unknown | progression from missing data | Software/product |
...
| --- | --- | --- | --- |
| Adherence/lifecycle | `src/adherence/__tests__/adherence.test.ts`, `sessionCompletionFeedback.test.ts`, `src/haleFlow/__tests__/appLifecycle.test.ts` | Date/progress state rules, completion feedback helpers, lifecycle outputs | Full `App.tsx` mutation order and remote restore races not tested |
...
| Workout/session planning | `src/training/__tests__/workoutGeneration.test.ts`, `src/haleFlow/__tests__/sessionPlanning.test.ts`, `src/training/__tests__/freshUser.integration.test.ts` | Generator and planning helpers | `App.tsx` passing all completions into planning; stale preview/start interactions |
...
| UI/view models | `src/components/__tests__/AccountAuthCard.test.ts`, `src/haleFlow/__tests__/*ViewModel.test.ts`, `src/navigation/__tests__/TabBar.test.ts`, `src/screens/__tests__/recordingViewport.test.ts` | Copy/view model and account UI components | Visual/voice/detector ID alignment in live flows |
...
| --- | --- | --- | --- | --- | --- | --- |
| R-001 | P0 | Onboarding/check-up | `App.tsx:1210-1339`, `src/screens/OnboardingResultsScreen.tsx:21-70`, `src/haleFlow/assessments.ts:68-76` | User can receive a plan from unmeasured or invalid baseline | Assessment confidence/status exists | Establish whether create-block must be gated by measured domains |
...
| R-004 | P1 | Dynamic planning | `App.tsx:1406-1438`, `src/haleFlow/sessionPlanning.ts:537-560`, `src/training/workoutGeneration.ts:307-347` | Micro-check/retest completion may advance session rotation/week state | `blockProgress` filters training sessions elsewhere | Confirm generator recentSessions must filter by session type |
| R-005 | P1 | Equipment/safety | `App.tsx:1741-1781`, `src/haleFlow/sessionPlanning.ts:646-671` | User's edited equipment may be silently overridden | Some handlers sync safety and training flags together | Audit all equipment edit paths and canonical source |
| R-006 | P1 | Assessment officialness | `src/haleFlow/assessments.ts:46-52`, `src/haleFlow/appLifecycle.ts:356-358` | Invalid official assessment can influence latest-assessment-dependent paths | Lifecycle filters completed official assessments | Align official/latest semantics |
...
| R-012 | P1 | Fallback planning | `src/haleFlow/sessionPlanning.ts:125-143`, `src/haleFlow/sessionPlanning.ts:501-535` | Dynamic failure may yield plausible but unintended legacy session | Fallback warning in dev | Define when fallback is acceptable in production |
| R-013 | P2 | Planning date | `src/haleFlow/sessionPlanning.ts:856-858` | Backdated/tests/week-boundary planning can get wrong plannedDateKey | Other date helpers accept `today` | Audit date/timezone handling |
...
| R-017 | P3 | Duplicated rules | `src/training/workoutGeneration.ts:964-982`, `src/haleFlow/sessionPlanning.ts:673-687`, `src/haleFlow/exploreViewModel.ts:235-260` | Drift between explore display and generator | Some tests cover helpers | Consolidation can be considered after correctness audit |
...
| Normal sign-out local data | Sign-out does not clear local Hale stores | Local-first product may intend device continuity, but multi-account trust risk is high | Should local data be scoped to user or cleared/hidden on sign-out? | Stage 8 |
...
- Files/modules: `App.tsx`, `src/haleFlow/*`, `src/exercises/*`, `src/training/block.ts`, `src/training/workoutGeneration.ts`.
...
- Files/modules: `src/scoring`, `src/haleFlow/assessments.ts`, `src/checkup/manualCheckup.ts`, result screens.
...
- Files/modules: `src/training/workoutGeneration.ts`, `src/haleFlow/sessionPlanning.ts`, `src/adherence/blockService.ts`.
...
- Files/modules: `App.tsx`, `src/adherence`, `src/haleFlow`, `src/training/progression.ts`, `src/training/validTimeProgression.ts`.
...
- Files/modules: test harnesses across `src/haleFlow`, `src/training`, `src/checkup`, `src/services/backend`.
