You are implementing Stage 5C of Pearl’s production-readiness work:
...
- `planTodayPearlSession` return contract;
...
```ts
type PearlSessionPlanningResult =
  | {
      kind: 'ready';
      plan: PearlSessionPlan;
...
- “Your previous plan needs to be refreshed before Pearl can safely plan today’s session.”
...
- “Pearl couldn’t safely prepare today’s session.”
...
- “Pearl needs an active current plan before it can prepare today’s session.”

### Legacy-only state

- “Your previous plan needs to be refreshed before Pearl can safely continue.”

### No safe exercises

- “Pearl couldn’t find a safe exercise combination for today’s setup.”
