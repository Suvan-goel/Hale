# Hale Unified Movement Check-Up Stage H1 - V1 Shell / V2 Adapter
...
- `npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h1-baseline-export` passed and the temp export directory was removed.

## Implementation

Added `src/screens/CheckUpRecordingShell.tsx`.

- Owns the shared polished recording presentation: full-screen safe pose view, skeleton viewport, Hale logo chrome, compact setup notice, help button, footer metric card, bottom controls, help modal, and discard modal.
...
- `npx --no-install expo export --platform all --output-dir /tmp/hale-unified-h1-export`: passed.
