You are carrying out Stage 5E.1 of Pearl’s production-readiness work:
...
3. Separate the Pearl mobile-app TypeScript project from the standalone `website/` TypeScript project without weakening either project’s checks.
...
```text
src/pearlFlow/__tests__/exploreViewModel.test.ts(241,7)
```
...
The root Pearl mobile-app TypeScript command currently sees website files and reports website-specific alias/implicit-any errors.
...
- the root Pearl app typecheck checks the mobile app, tests, relevant local modules, and intended root scripts;
...
`npm run typecheck` at the Pearl app root must remain the authoritative app release gate.
...
## Step 5: Establish an explicit Pearl app TypeScript boundary
