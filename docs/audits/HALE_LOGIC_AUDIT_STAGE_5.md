# Hale Logic Audit Stage 5: Dynamic Workout Generation And Completion Semantics
...
```text
 ...
 src/haleFlow/exploreViewModel.ts                 | 118 +++--
...
App.handleStartSession
  ...
  -> adaptGeneratedSessionToHaleSessionPlan
```
...
- `src/haleFlow/sessionPlanning.ts:557-580` adapts both adherence completions and generated session
...
- Completion adapter has no `sessionType` filter: `src/haleFlow/sessionPlanning.ts:557-566`.
- Generated summary adapter has no source filter: `src/haleFlow/sessionPlanning.ts:568-580`.
...
- A user can do a micro-check, a mobility reset, and a manual practice, then Hale can say the planned
...
- `src/haleFlow/__tests__/sessionPlanning.test.ts` verifies `countsTowardMainPlan` for preset/manual,
...
- Hale can advance the block, generate milestones, and eventually trigger retest after sessions where
...
- Missing result item defaults to completed: `src/haleFlow/sessionPlanning.ts:594-614`.
...
- Call `planTodayHaleSession({ today: someExplicitDate })`, or generate for a target day around
  midnight/time-zone boundaries, then complete the session.

Runtime chain:

```text
planTodayHaleSession(input.today)
...
- `today` is passed to the raw generator: `src/haleFlow/sessionPlanning.ts:107-120`.
- Adapter discards it for `plannedDateKey`: `src/haleFlow/sessionPlanning.ts:274-279`.
- `plannedDateKey` calls `dateKey(new Date())`: `src/haleFlow/sessionPlanning.ts:861-863`.
...
- Fallback is entered on empty/unsupported/errors: `src/haleFlow/sessionPlanning.ts:127-146`.
- Fallback uses legacy `nextSessionExercises`: `src/haleFlow/sessionPlanning.ts:521-554`.
...
- `countsTowardMainPlan` does not exclude `legacy_fallback`: `src/haleFlow/sessionPlanning.ts:289-294`.
...
generateTodaySession
  ...
  -> adaptGeneratedSessionToHaleSessionPlan still returns exercises if any are playable
...
- Main-plan credit checks only source/session type: `src/haleFlow/sessionPlanning.ts:289-294`.
...
- `availableEquipmentFor` starts from safety profile, then mutates it using training equipment:
  `src/haleFlow/sessionPlanning.ts:666-694`.
...
Expo config resolved `name: Hale`, `slug: hale`, `scheme: hale`, SDK `56.0.0`, iOS
`com.suvangoel.hale`, Android `com.suvangoel.hale`, and 6 plugins.
...
6. Pass the explicit planning date through `adaptGeneratedSessionToHaleSessionPlan` and
   `plannedDateKey`.
