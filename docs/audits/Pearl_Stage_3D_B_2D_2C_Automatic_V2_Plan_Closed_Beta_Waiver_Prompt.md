...
- describing Pearl as physically validated;
- describing Pearl as beta-ready for broad distribution.

## Central product rule

The plan is created automatically.

The user does not manually ask Pearl to build, create, generate, or personalise the plan.
...
-> Pearl automatically creates or reuses the 4-week block locally
...
This stage integrates frozen V2 assessment artifacts with Pearl’s verified Stage 5 training lifecycle.
...
- Balance Pearl task bands may drive Pearl-band focus.
...
```bash
rm -rf /tmp/pearl-stage3db2d2c-waiver-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-stage3db2d2c-waiver-export
rc=$?
rm -rf /tmp/pearl-stage3db2d2c-waiver-export
exit $rc
```
...
Do not declare Pearl broadly beta-ready or physically validated.
