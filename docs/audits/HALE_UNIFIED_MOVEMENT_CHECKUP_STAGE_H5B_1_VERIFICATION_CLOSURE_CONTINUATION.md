# Hale Unified Movement Check-Up - Stage H5B.1 Verification Closure Continuation
...
```text
/tmp/hale-h5b1-audio-signoff-before.sha256
```
...
```text
377 /tmp/hale-h5b1-audio-signoff-before.sha256
```
...
Source inspection confirmed that both safety and Movement Profile V2 fingerprints include canonical cue text, Hale voice ID, provider voice ID, provider name, generation model, output format, material voice settings, and schema/policy version. The verifier recalculates these fingerprints from current source and compares them with generated metadata.
...
Result: passed with Expo SDK `56.0.0`, platforms `ios` and `android`, app name `Hale`, slug `hale`, bundle/package `com.suvangoel.hale`.
...
```bash
rm -rf /tmp/hale-unified-h5b1-signoff-export
npx --no-install expo export \
  --platform all \
  --output-dir /tmp/hale-unified-h5b1-signoff-export
rc=$?
rm -rf /tmp/hale-unified-h5b1-signoff-export
exit $rc
```
...
```text
377 /tmp/hale-h5b1-audio-signoff-after.sha256
