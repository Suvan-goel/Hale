# Hale Logic Remediation - Stage 4B
...
```text
 ...
 src/haleFlow/__tests__/exploreViewModel.test.ts  |  65 ++++
 src/haleFlow/__tests__/sessionPlanning.test.ts   |  95 ++++-
 src/haleFlow/exploreViewModel.ts                 | 113 ++++--
...
`GeneratedExercise` now carries `intendedDomain`, `stimulusRole`, and `stimulusReason`. `HaleSessionPlanMetadata`, generated exercise metadata, local generated-session summaries, and serializers preserve these fields.
...
- Hale does not count shoulder mobility or overhead pressing as primary upper-pull work.
- Session guidance says: "Upper-back pulling was skipped because it needs a resistance band. Hale did not replace it with shoulder mobility or another movement."
...
```text
npm test -- --runInBand src/haleFlow/__tests__/sessionPlanning.test.ts src/haleFlow/__tests__/exploreViewModel.test.ts
Test Suites: 2 passed, 2 total
...
```text
npm test -- --runInBand src/scoring/__tests__/focusSelection.test.ts src/scoring/__tests__/scoreSnapshot.test.ts src/haleFlow/__tests__/assessmentEligibility.test.ts src/haleFlow/__tests__/copyGuardrails.test.ts src/haleFlow/__tests__/progressViewModel.test.ts
Test Suites: 5 passed, 5 total
