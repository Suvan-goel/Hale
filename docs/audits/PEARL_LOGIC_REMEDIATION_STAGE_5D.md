# PEARL LOGIC REMEDIATION STAGE 5D
...
```text
 ...
 src/pearlFlow/__tests__/sessionPlanning.test.ts     | 557 ++++++++++++++-
 src/pearlFlow/appLifecycle.ts                       |  37 +-
 src/pearlFlow/mainPlanEvents.ts                     |  11 +
 src/pearlFlow/sessionPlanning.ts                    | 757 +++++++++++++++++----
...
- `updateExerciseProgressionFromSession` in `src/pearlFlow/sessionPlanning.ts` called `updateLadderProgressAfterSession` directly after feedback.
...
```bash
npm test -- --runInBand src/pearlFlow/__tests__/progressionEvidence.test.ts src/pearlFlow/__tests__/sessionPlanning.test.ts src/training/__tests__/store.test.ts src/services/backend/__tests__/trainingStateSyncService.test.ts src/services/backend/__tests__/restoreService.test.ts
```
...
- Stage 5C current-planner authority: `sessionPlanning`, `appLifecycle`, `pearlFlow`;
- Stage 4A/4B progression/stimulus semantics: `workoutGeneration`, `validTimeProgression`, `progression`;
- Stage 3D focus regression: `focusStimulusEvidence`, `pearlFlow`.
