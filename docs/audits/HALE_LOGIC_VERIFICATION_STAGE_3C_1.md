# Hale Logic Verification - Stage 3C.1
...
```sh
npm test -- --runInBand src/scoring/__tests__/scoreSnapshot.test.ts src/haleFlow/__tests__/assessmentEligibility.test.ts src/history/__tests__/history.test.ts src/services/backend/__tests__/checkupSyncService.test.ts src/services/backend/__tests__/restoreService.test.ts src/haleFlow/__tests__/checkupHistory.test.ts src/haleFlow/__tests__/progressViewModel.test.ts
```
