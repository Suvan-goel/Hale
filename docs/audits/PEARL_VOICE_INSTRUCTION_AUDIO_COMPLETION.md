# Pearl Voice Instruction Audio Completion
...
No runtime TypeScript contract change was needed in this pass. The active generation source in `docs/audits/PEARL_VOICE_V2_1_FINAL_CUE_REGISTRY.csv` was corrected for exactly two cue IDs:
...
Pre-existing dirty files in Pearl Flow, screen wiring, movement-profile voice runtime, and micro-check runtime were preserved and are still visible in final Git status.
...
- `docs/audits/PEARL_VOICE_INSTRUCTION_AUDIO_COMPLETION.md`
...
```sh
rm -rf /tmp/pearl-voice-audio-completion-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-voice-audio-completion-export
rc=$?
rm -rf /tmp/pearl-voice-audio-completion-export
exit $rc
```
...
```sh
rm -rf /tmp/pearl-voice-audio-completion-safe-beta-export
env \
  ...
  npx --no-install expo export --platform all --output-dir /tmp/pearl-voice-audio-completion-safe-beta-export
rc=$?
rm -rf /tmp/pearl-voice-audio-completion-safe-beta-export
exit $rc
```
