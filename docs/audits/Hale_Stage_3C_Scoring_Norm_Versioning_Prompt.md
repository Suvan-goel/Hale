You are implementing Stage 3C of Hale’s production-readiness work:
...
When Hale scores a Check-Up, the resulting interpretation must be frozen with the Check-Up record.
...
Because Hale has not launched publicly, fail closed rather than guessing.
...
The report should calmly state that a direct comparison is unavailable because Hale’s scoring method changed.
