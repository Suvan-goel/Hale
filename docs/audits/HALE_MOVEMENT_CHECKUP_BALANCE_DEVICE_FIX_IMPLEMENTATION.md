# Hale Movement Check-Up Balance Device Fix Implementation
...
- Command used the required `/tmp/hale-movement-checkup-balance-fix-export` output dir and removed it afterward.
- Result: passed.
...
- Command used the required safe-beta env overrides with diagnostics/internal/legacy flags disabled, exported to `/tmp/hale-movement-checkup-balance-fix-safe-beta-export`, and removed it afterward.
- Result: passed.
- Counts: Assets 1592; web bundles 2; iOS bundles 1; Android bundles 1; files 2.
- Warning: Sentry org/project config missing; environment variables used as fallback.
