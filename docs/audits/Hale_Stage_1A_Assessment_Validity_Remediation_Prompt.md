You are implementing Stage 1A of Hale’s production-readiness work:
...
- a strength-focused training plan despite Hale having obtained no reliable measurement evidence

This is a P0 trust defect.

Hale’s camera is presented as a measuring instrument. The app must never claim to have identified a training focus or create a personalised block when no valid focus was measured.
...
2. `score.weakestDomain` is null, undefined, unsupported, or cannot be explicitly mapped to a Hale movement domain.
...
Use an appropriate existing Hale flow/scoring module or create a narrowly scoped module such as:
...
Use calm Hale copy, for example:
...
- Record a safe diagnostic using Hale’s existing observability infrastructure where practical.
