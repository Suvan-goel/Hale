# Hale Logic Remediation - Stage 3D-C
...
- `npx jest src/scoring/__tests__/focusSelection.test.ts src/scoring/__tests__/scoreSnapshot.test.ts src/haleFlow/__tests__/assessmentEligibility.test.ts src/services/backend/__tests__/checkupSyncService.test.ts src/services/backend/__tests__/restoreService.test.ts --runInBand`
