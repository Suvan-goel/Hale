...
See `docs/audits/PEARL_MPV2_RUNTIME_TIMELINES.csv` rows for `full_normal_mpv2_checkup_clara`. In the wait-for-speech normal path, cues play in order, but chair active still starts from the internal timer before an explicit spoken go.

## 8. Full Normal Marcus Timeline

See `docs/audits/PEARL_MPV2_RUNTIME_TIMELINES.csv` rows for `full_normal_mpv2_checkup_marcus`. Marcus has different asset durations but the same outcome classes as Clara.
...
See `docs/audits/PEARL_MPV2_RUNTIME_REMEDIATION_PLAN.md`.
