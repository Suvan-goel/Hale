# Pearl Logic Remediation Stage 3D-B.2D.2C
...
git diff --check
npx --no-install expo export --platform ios --output-dir /tmp/pearl-stage3db2d2c-waiver-export
```
...
- `src/pearlFlow/blockTrainingPlan.ts`
- `src/pearlFlow/movementProfileV2Block.ts`
- `src/pearlFlow/__tests__/movementProfileV2Block.test.ts`
...
```text
npm test -- --runInBand src/pearlFlow/__tests__/movementProfileV2Block.test.ts
npm test -- --runInBand src/pearlFlow/__tests__/movementProfileV2Block.test.ts src/pearlFlow/__tests__/appLifecycle.test.ts
npm run typecheck
```
...
npx --no-install expo config --type public
git diff --check
npx expo export --platform ios --output-dir /tmp/pearl-stage3db2d2c-waiver-export
```
