# Codex Prompt: Hale Training Floor-Transfer Readiness and Final-Position Gate

Read this entire prompt before changing anything.

Hale’s step-up alternation software/runtime phase is now closed at the software/static level.
...
However, Hale already appears to contain a canonical movement-capability system from earlier safety work. Current source is expected to include:
...
4. ensure Hale never says the user is ready before they are actually settled in the floor start position;
...
Do not imply that Hale can assist the transfer.
...
- `docs/audits/HALE_LOGIC_REMEDIATION_STAGE_4C_R.md`, if present
...
1. Hale has exactly one authoritative movement-capability source for floor transfer.
...
30. If the camera cannot robustly verify a start posture, Hale uses explicit user confirmation plus stable visibility/readiness rather than false confidence.
...
Use Hale’s premium, calm component system.
...
1. `docs/audits/HALE_TRAINING_FLOOR_READINESS_IMPLEMENTATION.md`
2. `docs/audits/HALE_TRAINING_FLOOR_READINESS_AUDIT.md`
3. `docs/audits/HALE_TRAINING_FLOOR_READINESS_AUDIT.json`
4. `docs/audits/HALE_TRAINING_FLOOR_READINESS_SCENARIOS.csv`
5. `docs/audits/HALE_TRAINING_FLOOR_CONTRACT_MATRIX.csv`
6. `docs/audits/HALE_VOICE_PROJECT_POST_FLOOR_READINESS_HANDOFF.md`
...
# Hale Training Floor-Transfer Readiness Implementation
