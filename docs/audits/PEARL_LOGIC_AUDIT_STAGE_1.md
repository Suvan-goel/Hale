# Pearl Production-Readiness Logic Audit - Stage 1
...
| Fallback-only production paths | 2 | Main dynamic fallback and preset fallback in `planTodayPearlSession`. |
...
- Pearl is not structurally ready for beta without Stage 1 remediation on identity/local data isolation and invalid-baseline block creation.
- Pearl is ready to proceed to Stage 2 measurement audit once Stage 2 explicitly scopes the production battery to `DEFAULT_BATTERY` and treats TUG as beta-hidden/historical-support only.
...
| Expo config | `app.json:1-46`; scheme `pearl`, camera permission, local pose-detection plugin, Sentry plugin. |
...
| `npx --no-install expo config --json >/tmp/pearl-expo-config-stage1.json && node -e '...'` | 0 | Expo public config parsed: name `Pearl`, slug `pearl`, scheme `pearl`, 6 plugins, iOS `com.suvangoel.pearl`, Android `com.suvangoel.pearl` | Temporary `/tmp` output deleted. |
| Lint | Not run | No repository lint script exists | Not applicable. |

Additional non-mutating diagnostics:

- Pure `tsx` imports that touched React Native through barrel exports failed with `react-native/index.js:27 Unexpected "typeof"`. This is an audit limitation, not a repo failure. It affected direct ad-hoc imports of `src/pearlFlow/sessionPlanning.ts` and `src/pearlFlow/assessments.ts`; the same code is covered by Jest/Babel.
...
| Non-training completions can influence dynamic planning | Confirmed | Confirmed software defect P1 | `App.tsx:1416-1433`, `src/pearlFlow/sessionPlanning.ts:537-560`, `src/training/workoutGeneration.ts:307-347`. | Filter by session type before generator recentSessions. |
| Null plan counts toward main plan | Function true, runtime launch narrowed | Stage 0 claim corrected / Latent defect | `countsTowardMainPlan(null)` at `src/pearlFlow/sessionPlanning.ts:282-287`; `beginPlannedSession` guards null/empty at `App.tsx:1489-1495`. | Keep callback-race audit for Stage 6/7; start button path is guarded. |
...
| Missing result can be completed | Confirmed | Latent defect P1 | `src/pearlFlow/sessionPlanning.ts:568-600` treats absent item as completed. | Require result/plan ID reconciliation. |
...
| Equipment has multiple sources | Confirmed | Product decision required P1 | Settings handlers `App.tsx:1741-1781`; generator merge `src/pearlFlow/sessionPlanning.ts:646-671`; Explore duplicate `src/pearlFlow/exploreViewModel.ts:235-260`. | Choose canonical equipment model. |
| Dynamic failure falls back silently | Confirmed | Fallback-only but production-reachable P1 | `src/pearlFlow/sessionPlanning.ts:101-143`, `src/pearlFlow/sessionPlanning.ts:501-535`; dev-only warning. | Add production observability/constraints or hard stop. |
| Planning date ignored | Confirmed for plannedDateKey only | Confirmed software defect P2 | `src/pearlFlow/sessionPlanning.ts:856-858`; other week helpers accept injected dates. | Use supplied `today` in planned date key. |
...
| P09 | Plan today's session | Today/Plan/block intro | CTA -> `handleStartSession` | `planTodayPearlSession` -> dynamic generation -> validation/fallback -> `activeSessionPlan` | safety, lifeGoal, activeBlock, training, completions | in-memory plan | active block or fallback/preset | Confirmed production-reachable | `App.tsx:1406-1438`, `src/pearlFlow/sessionPlanning.ts:94-191` |
...
| P21 | Sign out | `AccountAuthCard` | `submitSignOut` -> `signOut` | Supabase signOut only; local Pearl files preserved | auth state | auth state only | signed in | Confirmed production-reachable | `src/components/AccountAuthCard.tsx:173-182`, `src/services/backend/AuthProvider.tsx:271-284` |
| P22 | Clear local data | Account data action | confirm -> `clearLocalPearlData` -> `signOut` | delete known local files and recordings | local files | deletes local stores | explicit destructive confirmation | Confirmed production-reachable | `src/components/AccountAuthCard.tsx:217-245`, `src/services/backend/accountDataService.ts:53-97` |
...
| P24 | Export data | Account UI | `submitExportData` -> `sharePearlDataExport` | local/remote export service | local/backend | share artifact outside repo | signed in | Confirmed production-reachable | `src/components/AccountAuthCard.tsx:184-202` |
...
| Active session plan | in-memory `PearlSessionPlan` | `handleStartSession`/practice | preview/training/completion/feedback | GeneratedSession -> PearlSessionPlan | none | session summary after completion | in-memory plan | callback races not idempotent | P1 |
...
| Camera orientation | exercise definition spec, ladder string | exercise modules/ladders | player/preflight/preview/Explore | GeneratedExercise -> PearlExercise | source only | none | player uses exercise definition; generator/preview uses ladder | 3 mismatches | P2 |
...
- Dynamic session content: `planTodayPearlSession -> generateTodaySession`.
...
sequenceDiagram
  ...
  participant Plan as planTodayPearlSession
...
| `legacyFallbackPlan` | `src/pearlFlow/sessionPlanning.ts:501-535` | planToday fallback branches | sessionPlanning tests | active block with generator failure | Fallback-only but production-reachable | dynamic generated plan | compatibility | P1 |
...
| `manualCheckup.ts` | `src/pearlFlow/manualCheckup.ts` | manual check-up UI/options | pearlFlow tests | manual route | Confirmed production-reachable | check-up type matrix | product semantics | P1 |
...
| C-015 | Account state | auth user changes | local Pearl stores fixed | local stores win before restore if meaningful | Production | user data bleed | P0 | Stage 8 |
...
`latestOfficialAssessment` filters only `isOfficialForProgress` and sorts by completed/created date (`src/pearlFlow/assessments.ts:46-52`). Lifecycle's `hasOfficialAssessment` does require `status === 'completed'` (`src/pearlFlow/appLifecycle.ts:356-358`). Therefore Stage 0 was too broad if applied to lifecycle. The risk remains for any direct caller of `latestOfficialAssessment` or code that derives latest from raw history.

Existing tests: pearlFlow tests assert official flags, not invalid-official exclusion across all callers.
...
Trace: `App.tsx:1416-1433` passes all `adherence.completions`; `recentSessionsFor` maps every completion to status completed at `src/pearlFlow/sessionPlanning.ts:537-560`; `getTemplateSelection` counts all non-skipped block sessions at `src/training/workoutGeneration.ts:312-347`.
...
`countsTowardMainPlan(null)` returns true at `src/pearlFlow/sessionPlanning.ts:282-287`. However `beginPlannedSession` now returns if `!activeSessionPlan || activeSessionPlan.exercises.length === 0` (`App.tsx:1489-1495`), and the renderer only mounts preview/training with plan/IDs (`App.tsx:2111-2123`).
...
`plannedDateKey(templateId)` uses `new Date()` at `src/pearlFlow/sessionPlanning.ts:856-858` instead of supplied `today`, while session ID/dateKey elsewhere accepts injected dates. Week selection in `workoutGeneration.getTemplateSelection` uses injected `today`; lifecycle date helpers use passed `today`.
...
Normal sign-out chain: `AccountAuthCard.submitSignOut -> AuthProvider.signOut -> signOutWithSupabase -> empty auth state`. No local Pearl stores are cleared, hidden, or re-keyed. On another account sign-in, App local hydration loads existing files before restore; restore skips if local state is meaningful.
...
| F-002 | P0 | High | Confirmed software defect | User A signs out, User B signs in on same device | Account sign-out -> local files preserved -> restore skips non-empty local | B can see A's local Pearl state | Local state user-scoped/cleared/hidden | Restore skip tested, not account switch | Identity/privacy/trust failure | Remediate before beta |
...
| F-005 | P1 | Medium | Confirmed software defect / fallback-only | Dynamic generation error/empty/unsupported | `planTodayPearlSession` -> `legacyFallbackPlan` | Silent legacy plan can bypass dynamic constraints | Fallback constrained/observable | Fallback test checks IDs only | Wrong exercise/pain/equipment risk | Contain fallback |
| F-006 | P2 | High | Confirmed software defect | Planning with injected date | `adaptGeneratedSessionToPearlSessionPlan` -> `plannedDateKey(new Date())` | plannedDateKey uses ambient date | Deterministic injected date | No test | Wrong duplicate/week key | Pass date through |
...
| Assessment selection (`pearlFlow.test.ts`) | manual/official flags, retake replacement | pure helpers | simplified assessments | Yes | Does not test raw history latest/manual block creation. |
...
| --- | --- | --- | --- | --- | --- |
| Assessment validity gate | F-001, INV-001, INV-005 | `App.tsx`, `src/pearlFlow/assessments.ts`, result screens/view models | invalid baseline matrix, onboarding CTA gating | Low/medium, affects first-run UX | First |
| Session-type filtering | F-003 | `src/pearlFlow/sessionPlanning.ts`, possibly adherence helpers | micro/retest/manual completion adapter tests | Low | First |
