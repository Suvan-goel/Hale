# Pearl Logic Remediation Stage 3D-D
...
- Existing modified files included app, adherence, Pearl flow, history, scoring, backend, screen, theme, native pose-view, and docs files.
- Existing untracked files included prior audit reports/prompts, `src/scoring/focusSelection.ts`, `src/scoring/scoreSnapshot.ts`, `src/scoring/versions.ts`, `src/checkup/retry.ts`, `src/pearlFlow/checkupHistory.ts`, `src/pearlFlow/assessmentEvidence.ts`, related tests, and Explore insight images.
...
```sh
npm test -- --runInBand src/scoring/__tests__/focusSelection.test.ts src/scoring/__tests__/scoreSnapshot.test.ts src/pearlFlow/__tests__/assessmentEligibility.test.ts src/adherence/__tests__/blockServiceEligibility.test.ts src/pearlFlow/__tests__/progressViewModel.test.ts src/pearlFlow/__tests__/copyGuardrails.test.ts src/services/backend/__tests__/blockReportSyncService.test.ts
```
