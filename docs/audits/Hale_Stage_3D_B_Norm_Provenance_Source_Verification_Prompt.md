You are carrying out Stage 3D-B of Hale’s production-readiness work:
...
Do not claim Hale is beta-ready in this task.
...
Stage 3D-B must determine whether Hale’s current normative tables, age-range transformations, cross-domain focus comparison, and user-facing estimate policy are scientifically supportable.
...
Do not quote long source passages. Paraphrase protocol/population details and reproduce only the minimum individual values needed to compare Hale’s code.
...
- Hale’s production single-leg stage caps the hold at 12 seconds;
- current code anchors include approximately 27–35 seconds for younger/younger-old groups;
- a perfect Hale 12-second trial cannot span the source scale appropriately.
...
7. Verify whether each source protocol matches Hale’s home-camera protocol closely enough for:
   - internal focus use;
...
Temporary downloads and scripts must stay under `/tmp/hale-stage3db/` and must be deleted before completion.
...
Use a temporary read-only script under `/tmp/hale-stage3db/` if helpful.
...
- no source;
- estimated or heuristic values created by Hale.
...
Compare the source protocol with Hale’s current assessment:

| Protocol dimension | Source | Hale | Match verdict |
| --- | --- | --- | --- |
| Test duration | | | |
| Chair height | | | |
| Backrest/seat | | | |
...
- source verified?
- protocol compatible with Hale’s room/home setup?
...
- Hale’s maximum single-leg stage duration;
...
- the best possible Hale valid score;
...
Do not select a source merely because its values look similar to Hale’s table.

If no exact primary source matches the code:

- classify the whole table as Hale-estimated/unsupported;
...
What can Hale safely say before device validation?
