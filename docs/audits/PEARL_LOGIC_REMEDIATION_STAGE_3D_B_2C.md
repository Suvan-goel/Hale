# PEARL LOGIC REMEDIATION STAGE 3D-B.2C
...
Added V2 official snapshot selectors in `src/pearlFlow/checkupHistory.ts`:
...
```bash
rm -rf /tmp/pearl-stage3db2c-export
npx --no-install expo export --platform all --output-dir /tmp/pearl-stage3db2c-export
rc=$?
rm -rf /tmp/pearl-stage3db2c-export
exit $rc
```
