# Hale Logic Audit Stage 3: Check-Up Scoring, Officialness, Progress, And Reports
...
Stage 3 does not clear Hale's scoring and official progress semantics for beta. The core scoring
...
| Assessment confidence | high >=3 measured, medium >=1, low 0 | `src/haleFlow/assessments.ts:90` |
| Assessment status | low -> invalid, otherwise completed by default | `src/haleFlow/assessments.ts:14` |
| Official type | baseline, baseline_retake, official_retest | `src/haleFlow/assessments.ts:50` |
| Block eligibility | completed assessment, measured domains >0, focus present, focus finite, consistent | `src/haleFlow/assessmentEligibility.ts:28` |
| Snapshot band | midpoint <=58 strong, <=72 building, else starting point | `src/haleFlow/appLifecycle.ts:429` |
| Report improvement | lower movement-age midpoint is improved; exact equality is held steady | `src/haleFlow/reports.ts:58` |
...
```text
npx --no-install tsx /tmp/hale-stage3-probe.ts
```
...
`src/haleFlow/__tests__/assessmentEligibility.test.ts:205`.
...
- `isOfficialCheckupType` returns true only for baseline, baseline_retake, official_retest
  (`src/haleFlow/assessments.ts:50`).
- Manual option copy says manual-extra will not replace official trend
  (`src/haleFlow/manualCheckup.ts:69` and `src/haleFlow/manualCheckup.ts:88`).
...
- `ProgressScreen` receives raw `history`, and `getLatestCheckUpSummary` scores the latest raw history
  item without a type filter (`src/haleFlow/progressViewModel.ts:86`).
- `getDomainProgressCards` compares first raw check-up to latest raw check-up (`src/haleFlow/progressViewModel.ts:97`).
- Lifecycle `latestUsableCheckUpScore` scans raw history backward and accepts the latest block-eligible
  raw score, independent of assessment officialness (`src/haleFlow/appLifecycle.ts:352`).
...
- If block eligible, says "Hale will use this to shape your first 4-week block"
...
- Balance progress uses tandem hold preferentially, then one-leg hold, then best stage
  (`src/haleFlow/progressViewModel.ts:322`), while scoring uses one-leg only. This means Progress can
...
- `MovementAssessment.results.rawMetrics` stores only `checkUpId` and measured-domain count
  (`src/haleFlow/assessments.ts:40`).
...
| Onboarding | "Hale will use this to shape your first 4-week block" | Eligibility | One measured domain currently enough. |
...
|---|---|---|---|---|---|
| F3-001 | P1 | Product decision / eligibility | One measured domain can create a completed, medium-confidence, block-eligible assessment and plan. | `src/haleFlow/assessments.ts:90`; `src/haleFlow/assessmentEligibility.ts:28`; probe strength-only and shoulder-only; test at `src/haleFlow/__tests__/assessmentEligibility.test.ts:205`. | High |
| F3-002 | P1 | Logic / officialness | Manual-extra can contaminate raw-history progress and next-block flows despite not being official. | `src/haleFlow/manualCheckup.ts:69`; `src/haleFlow/progressViewModel.ts:86`; `src/haleFlow/appLifecycle.ts:352`; `App.tsx:1912`. | High |
...
| F3-004 | P1 | Versioning | Scoring/norm version is not pinned to history, assessments, reports, or restore. | `src/history/serialize.ts:4`; `src/haleFlow/assessments.ts:40`; `src/services/backend/restoreService.ts:485`. | High |
...
| F3-008 | P2 | Reporting | Progress/report changes have no meaningful-change thresholds. | `src/haleFlow/reports.ts:58`; `src/adherence/screens/BlockReportScreen.tsx:108`; `src/screens/ResultsScreen.tsx:166`. | High |
