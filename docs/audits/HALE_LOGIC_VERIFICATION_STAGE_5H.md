# Hale Logic Verification - Stage 5H
...
Stage 5H re-verified Hale's current dynamic training lifecycle as an integrated system:
...
Can Hale reliably transform a valid official Movement Check-Up into four weeks of safe, deterministic, credit-honest training, survive retries/restore/offline-like ordering, complete one official re-test transition, and create one next block without user-facing overclaim?
...
```text
 App.tsx                           | 21 +++++++++++----------
 src/haleFlow/appLifecycle.ts      |  1 -
 src/haleFlow/blockSchedule.ts     | 17 ++++++++++++-----
 src/haleFlow/index.ts             |  1 +
 src/haleFlow/progressViewModel.ts |  2 +-
...
- `planTodayHaleSession` is scheduler-first: it asks `getBlockScheduleState` for due template/week/retest state, then generates a dynamic current session with canonical equipment.
...
- generated plans through real `planTodayHaleSession`;
...
| I121 | Sign-out local clear removes main Hale files. | Holds | Stage 5H account | Cloud deletion deferred |
...
- Production remediation: `src/haleFlow/nextBestAction.ts` now checks `schedule.status === 'block_completed'`.
- Regression test: `src/haleFlow/__tests__/haleFlow.test.ts` now requires retest completion for report-ready and verifies stale completed flag does not overclaim.
...
- Added `src/haleFlow/__tests__/stage5hLifecycle.integration.test.ts`
- Updated `src/haleFlow/__tests__/haleFlow.test.ts`
...
```text
 ...
 M src/haleFlow/blockSchedule.ts
 M src/haleFlow/index.ts
 M src/haleFlow/nextBestAction.ts
 M src/haleFlow/progressViewModel.ts
...
```text
 ...
 src/haleFlow/__tests__/appLifecycle.test.ts    |  22 ++-
 src/haleFlow/__tests__/haleFlow.test.ts        |  34 +++-
 src/haleFlow/__tests__/sessionPlanning.test.ts |  21 +++
 src/haleFlow/appLifecycle.ts                   |  11 +-
 src/haleFlow/blockSchedule.ts                  |  17 +-
 src/haleFlow/index.ts                          |   1 +
 src/haleFlow/nextBestAction.ts                 |   2 +-
 src/haleFlow/progressViewModel.ts              |   2 +-
 src/haleFlow/sessionPlanning.ts                |   4 +-
...
- `src/haleFlow/nextBestAction.ts` - production fix for F5H-001.
- `src/haleFlow/__tests__/haleFlow.test.ts` - regression coverage for F5H-001.
- `src/haleFlow/__tests__/stage5hLifecycle.integration.test.ts` - new Stage 5H suite.
- `docs/audits/HALE_LOGIC_VERIFICATION_STAGE_5H.md` - this report.
