# Pearl V1 App Structure Audit

> **Historical audit — not current product authority.** This June 2026 map predates the
> promoted 12-week programme and fixed Strength-and-Balance check-up. Use `AGENTS.md` for the
> current audience and MVP; use this file only when tracing retired implementation history.
...
- Route strings used by `src/pearlFlow/nextBestAction.ts` are interpreted by `handleRoute` in `App.tsx`.
...
- `WelcomeScreen` already frames Pearl as a private Movement Check-Up and 4-week block.
...
- `src/pearlFlow/sessionPlanning.ts` adapts the dynamic generator into `PearlSessionPlan`.
- `src/pearlFlow/sessionPlanning.ts` is not currently called by `App.tsx` or the live UI.
- The current UI therefore still runs the older block/session resolver, even though the dynamic generator exists.

Recommended connection:

- Generate "Today's Pearl Session" from `src/pearlFlow/sessionPlanning.ts` using the active `MovementBlock`, safety profile, life goal, adherence state, readiness, and recent completions.
...
- `src/screens/HomeScreen.tsx`
  ...
  - Lead with "Today's Pearl Session" / next best action.
...
- `src/pearlFlow/copy.ts`, `src/adherence/adherenceCopy.ts`
...
### Stage 2: Dynamic Today's Pearl Session Integration
...
- `App.tsx`
  - Add a `todaySessionPlan` derived from `generateTodaySession` in `src/pearlFlow/sessionPlanning.ts`.
...
- `src/pearlFlow/sessionPlanning.ts`
  - Confirm generated `PearlSessionPlan` carries all metadata needed for display and completion.
...
- Tests:
  - Add/extend `src/pearlFlow/__tests__/pearlFlow.test.ts` for UI-facing session planning.
