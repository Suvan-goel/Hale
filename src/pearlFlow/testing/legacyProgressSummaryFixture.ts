...
export function getRetestDueSummary({
  ...
  if (!hasBaseline) {
    return {
      ...
      body: 'Do one Movement Check-Up so Pearl can save your starting numbers.',
  ...
  if (schedule.status === 'retest_due') {
    return {
      ...
      body: 'Repeat the same Movement Check-Up now so Pearl can compare it with your last one.',
  ...
  if (days === 0) {
    return {
      title: 'Next check-up',
      body: 'Finish the planned sessions in this 4-week plan, then Pearl will open your next check-up.',
      due: false,
    };
  }
  return {
